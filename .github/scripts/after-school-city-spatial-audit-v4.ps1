...
# AFTER SCHOOL CITY — CLOUD SOURCE SPATIAL AUDIT V4
# Fail-closed source parser + effective X/Z AABB clearance model after v0.4.7 orientation correction.
...
Token $orient 'root:WaitForChild("V046_SourceSpatialFix", 20)' 'v0.4.7 order'
Token $config 'Version = "0.4.7-orientation-correction-1"' 'v0.4.7 config'
Token $config 'EnableOrientationCorrectionPass = true' 'v0.4.7 flag'
Token $project 'ASC_OrientationCorrection' 'v0.4.7 Rojo wiring'

# Roads parsed from generator / effective override source.
LiteralPart $world 'EastWestRoad' 'road' 'world'
$roadSizeZ=Const 'MAIN_ROAD_SIZE_Z'; $roadCenterZ=Const 'MAIN_ROAD_CENTER_Z'
Box 'NorthSouthRoad' 'road' 0 $roadCenterZ 40 $roadSizeZ 'v0.4.7'
...
# Walkable/access envelopes. Main-spine sidewalks follow the v0.4.7 road length.
Box 'NS_Sidewalk_W' 'path' -24.5 $roadCenterZ 9 $roadSizeZ 'v0.4.7'; Box 'NS_Sidewalk_E' 'path' 24.5 $roadCenterZ 9 $roadSizeZ 'v0.4.7'
...
# School core and sports remain fixed; v0.4.7 mirrors frontage elements only.
...
# v0.4.6 shop contract is retained by v0.4.7.
...
foreach($s in $shops){ Box $s.Groups[1].Value 'building' (N $s.Groups[2].Value) $shopZ (N $s.Groups[3].Value) $shopDepth 'v0.4.6/v0.4.7' }

# Other effective building footprints.
...
# Trees: parse six park source vectors, then apply exact v0.4.6 relocation table.
...
Token $clearance 'return pos.Z >= 214 and pos.Z <= 286 and math.abs(pos.X) <= 116' 'legacy north school tree hard-clear zone'
Token $orient 'SCHOOL_FRONT_CLEAR_Z_MIN = 145' 'new south school clear-zone min'; Token $orient 'SCHOOL_FRONT_CLEAR_Z_MAX = 180' 'new south school clear-zone max'
$notes.Add('v0.4.5 preserves the former north courtyard tree clear zone; v0.4.7 additionally clears the measured south arrival apron.')|Out-Null
...
$frontX=Const 'SCHOOL_FRONT_CLEAR_X'; $frontMin=Const 'SCHOOL_FRONT_CLEAR_Z_MIN'; $frontMax=Const 'SCHOOL_FRONT_CLEAR_Z_MAX'
Box 'SchoolMainDoorCirculationV047' 'zone' 0 (($frontMin+$frontMax)/2) ($frontX*2) ($frontMax-$frontMin) 'v0.4.7-policy';$door=$boxes[$boxes.Count-1];foreach($o in ($trees+$hedges+$vehicles)){if(Hit $o $door){Fail 'school-entrance-circulation' $o $door 'object enters v0.4.7 south entrance clear zone'}}
...
[ordered]@{project='AFTER SCHOOL CITY';audit='CLOUD_SOURCE_SPATIAL_AUDIT_V4';sourceVersion='0.4.7-orientation-correction-1';status=$status;counts=$counts;notes=@($notes);findings=@($findings);geometry=@($boxes)}|ConvertTo-Json -Depth 8|Set-Content 'spatial-audit-after-school-city.json' -Encoding UTF8
Write-Host 'AFTER SCHOOL CITY — CLOUD SOURCE SPATIAL AUDIT V4'
...
Write-Host 'PASS: no modeled collision/clearance conflicts after effective v0.4.7 source corrections'
