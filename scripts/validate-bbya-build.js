...
const zoneFiles=[
'maps/a-club/v5/00-core.lua','maps/a-club/v5/10-design-system.lua','maps/a-club/v5/15-component-registry.lua',
...
const uiFiles=['maps/a-club/v5/ui-shell.client.lua','maps/a-club/v5/ui-component-inspector.client.lua','maps/a-club/v5/ui-shell-polish.client.lua','maps/a-club/v5/ui-inspection-nav.client.lua','maps/a-club/v5/ui-floating-dock.client.lua','maps/a-club/v5/ui-container-dock.client.lua','maps/a-club/v5/ui-live.client.lua','maps/a-club/v5/ui-performance.client.lua'];
...
for(const code of ['01','02','03','04','05','06','07','08','09W','09E','10','11W','11E','12W','12E','13','14','15','16W','16E','17','18','19','20','21','22','23'])if(!architecture.includes(`registerComponent("${code}"`))fail(`component registration missing: ${code}`);
for(const marker of ['BBYAV5DesignSystem','BBYAV5ComponentSchema','BBYAV5ComponentCount','PREMIUM EXTERIOR','PREMIUM MAIN CLUB','PREMIUM INFINITY POOL','PREMIUM SKY BAR','PREMIUM CABANAS','BBYAV5Atmosphere','5.3-master-plan','BBYAV5WorldInspectionTags','BBYA_V5_InspectionNav','CODED_SAFE_LANDINGS'])if(!architecture.includes(marker))fail(`architecture/finish marker missing: ${marker}`);
...
for(const marker of ['BBYA_V5_UI','BBYACurrentZone','BBYACurrentComponent','BBYAUIInspectionSchema','MACRO_PLUS_COMPONENT','BBYAUIFloatingDock','BBYAUIContainerDock','BBYAUILiveSystems','BBYAClientPerformanceProfile','BBYAV5TPPanel','TOP_UP/LEFT_LEFT/RIGHT_RIGHT/PEEK_ONLY'])if(!ui.includes(marker))fail(`UI marker missing: ${marker}`);
...
if(target&&exists(target.file)){
 ...
 if(injected){const begin='<!-- BBYA_RUNTIME_BEGIN -->',end='<!-- BBYA_RUNTIME_END -->';const bc=xml.split(begin).length-1,ec=xml.split(end).length-1;if(bc!==1||ec!==1)fail(`runtime markers invalid begin=${bc} end=${ec}`);const runtime=bc===1&&ec===1?xml.slice(xml.indexOf(begin),xml.indexOf(end)+end.length):'';
  ...
  for(const code of ['01','05','06','07','12W','12E','13','14','15','17','23'])if(!runtime.includes(`registerComponent("${code}"`))fail(`injected runtime missing component ${code}`);
  for(const marker of ['5.3-master-plan','BBYAV5Atmosphere','BBYASystemMusic','BBYASystemMoney','BBYASystemLift','BBYAUILiveSystems','BBYAUIFloatingDock','BBYAUIContainerDock','BBYAClientPerformanceProfile','MACRO_PLUS_COMPONENT'])if(!runtime.includes(marker))fail(`injected master marker missing: ${marker}`);
  ...
  if(!failed)pass('post-injection V5.3 master runtime + two-level inspection addressing valid');
 }
}
if(failed){console.error(`[BBYA VALIDATE] ${injected?'POST-INJECTION':'SOURCE'} BUILD REJECTED`);process.exit(1)}
console.log(`[BBYA VALIDATE] ${injected?'POST-INJECTION':'SOURCE'} CHECKS PASSED • V5.3 MASTER PLAN • MACRO+MICRO CODES • LIVE SYSTEMS • UNIFIED UI`);
