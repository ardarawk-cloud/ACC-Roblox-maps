...
for (const marker of ['MOUNT_BBYA_V12_World','MOUNT_BBYA_V12_MobileCamera','mobile-control-hotfix-v1.2','CharacterAutoLoads=true']) {
  if (!body.includes(Buffer.from(marker))) throw new Error(`Generated place marker absent: ${marker}`);
}
if (body.includes(Buffer.from('Players.CharacterAutoLoads=false'))) throw new Error('CharacterAutoLoads=false detected in v1.2 place');
...
(async()=>{
  ...
  if(!response.ok){console.error('MOUNT BBYA v1.2 publish failed',response.status,payload);process.exit(1)}
  console.log('MOUNT BBYA v1.2 publish success',payload);
})();
