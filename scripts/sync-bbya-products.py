...
DESIRED=[
 ...
 ('BBYA Skatepark Travel',5,'Teleport this visit to the BBYA Skatepark.'),
...
if err or not st.startswith('2'):
 ...
 result['api_ok']=True
 # Exact-name resolution is required because multiple BBYA products intentionally share prices.
 ...
 # Preserve two previously verified legacy support IDs if listing ever omits them transiently.
...
lines=['-- Generated by scripts/sync-bbya-products.py','return {',f"    DJ_MESSAGE = {ids.get('BBYA DJ Wall Message',0)},",'    TRAVEL = {',f"        VIP = {ids.get('BBYA VIP Travel',0)},",f"        Skatepark = {ids.get('BBYA Skatepark Travel',0)},",f"        Rooftop = {ids.get('BBYA Rooftop Travel',0)},",f"        Basement = {ids.get('BBYA Basement Travel',0)},",'    },','    SUPPORT = {']
