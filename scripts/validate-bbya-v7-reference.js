...
requireTokens('architecture',[
  'LEFT CLUB WING — 3-TIER HERO MASS','CLUB GROUND SLAB','MEZZ LEVEL 1','MEZZ LEVEL 2','DANCE FLOOR','MAIN STAGE','DJ BOOTH','LED WALL',
  'CENTER SOCIAL ATRIUM','VIP FLOOR','ROOFTOP DECK','POOL BASIN','POOL WATER','POOL DJ ISLAND','STAIR G TO MID','STAIR MID TO ROOF',
  'MAIN BBYA WORDMARK','BBYAHeroComposition','LEFT_CLUB_CENTER_SOCIAL_RIGHT_VIP_UPPER_POOL'
],'architecture');
pass('reference hero massing present');

requireTokens('exterior',['ATRIUM HERO GLASS','ATRIUM HERO CANOPY','CLUB FACADE FIN','CLUB TOP CORNICE PINK','VIP FACADE PANEL','VIP PORTAL BRAND','ROOF PERGOLA POST','POOL PARTY BILLBOARD','HERO PALM','HERO PLANTER','BBYAReferenceSilhouette'],'exterior');
...
if(/maps\/a-club\/v[0-9]+\//.test(src.assembler)) fail('archived versioned source leaked into assembler'); else pass('deterministic clean assembler isolated from archived builds');
