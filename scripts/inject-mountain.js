...
const serverModules = [
  ...
  ['microdetail','maps/mountain-social/mountain.microdetail.server.lua'],
  ['environment','maps/mountain-social/mountain.environment.server.lua'],
  ['immersive','maps/mountain-social/mountain.immersive.server.lua'],
  ['worldlife','maps/mountain-social/mountain.worldlife.server.lua'],
...
const qc = `task.delay(82,function()\n local r=workspace:FindFirstChild('ACC_MountainSocial')\n local cps=r and r:FindFirstChild('Checkpoints')\n local ok=r~=nil\n  and r:GetAttribute('WorldRebuild')=='4.2'\n  and r:GetAttribute('PrecisionRoadReady')==true\n  and r:GetAttribute('PrecisionVersion')=='4.4'\n  and r:GetAttribute('LowerForestReady')==true\n  and r:GetAttribute('RiverCrossingReady')==true\n  and r:GetAttribute('MidCampReady')==true\n  and r:GetAttribute('CliffZoneReady')==true\n  and r:GetAttribute('FogZoneReady')==true\n  and r:GetAttribute('HighlandReady')==true\n  and r:GetAttribute('RidgeReady')==true\n  and r:GetAttribute('FalseSummitReady')==true\n  and r:GetAttribute('SummitScenicReady')==true\n  and r:GetAttribute('TerrainGroundingReady')==true\n  and r:GetAttribute('TerrainGroundingVersion')=='4.6'\n  and (r:GetAttribute('GroundedSceneryCount') or 0)>0\n  and r:GetAttribute('RealismPassReady')==true\n  and r:GetAttribute('RealismVersion')=='4.7'\n  and r:GetAttribute('GroundedRealism')==true\n  and (r:GetAttribute('TerrainBreakupCount') or 0)>0\n  and (r:GetAttribute('NaturalRockCount') or 0)>0\n  and r:GetAttribute('MicroDetailReady')==true\n  and r:GetAttribute('MicroDetailVersion')=='4.8'\n  and (r:GetAttribute('DetailedTreesReplaced') or 0)>0\n  and (r:GetAttribute('SagWireSpans') or 0)>0\n  and r:GetAttribute('EnvironmentRealismReady')==true\n  and r:GetAttribute('EnvironmentRealismVersion')=='4.9'\n  and (r:GetAttribute('ErosionPatchCount') or 0)>0\n  and (r:GetAttribute('RoadWeatherDetailCount') or 0)>0\n  and (r:GetAttribute('MixedVegetationCount') or 0)>0\n  and (r:GetAttribute('IrregularOutcropCount') or 0)>0\n  and r:GetAttribute('ImmersiveBiomeReady')==true\n  and r:GetAttribute('ImmersiveBiomeVersion')=='5.0'\n  and (r:GetAttribute('RoadCrackDetailCount') or 0)>0\n  and (r:GetAttribute('TrailRootCount') or 0)>0\n  and (r:GetAttribute('RiverMicroDetailCount') or 0)>0\n  and (r:GetAttribute('HighlandBiomeDetailCount') or 0)>0\n  and (r:GetAttribute('NightLanternCount') or 0)>0\n  and r:GetAttribute('WorldLifeReady')==true\n  and r:GetAttribute('WorldLifeVersion')=='5.1'\n  and (r:GetAttribute('VillageLifeDetailCount') or 0)>0\n  and (r:GetAttribute('IrrigationDetailCount') or 0)>0\n  and (r:GetAttribute('ForestDepthDetailCount') or 0)>0\n  and (r:GetAttribute('TrailRetainingDetailCount') or 0)>0\n  and (r:GetAttribute('SummitPolishDetailCount') or 0)>0\n  and (r:GetAttribute('NightFireflyEmitterCount') or 0)>0\n  and workspace:GetAttribute('ACC_TimeCycle')=='v4.1-four-phase'\n  and cps~=nil and #cps:GetChildren()>=12\n workspace:SetAttribute('ACC_MountainReady',ok)\n workspace:SetAttribute('ACC_MountainBuild','v5.1-world-life')\n if r then r:SetAttribute('BuildVersion','5.1.0-world-life') end\n if not ok then warn('[Mountain:QC] v5.1 world-life readiness gate failed') end\nend)`;
...
console.log('[Mountain] Injected ACC Mountain v5.1 world-life realism into',target.file,'bytes',Buffer.byteLength(xml));
