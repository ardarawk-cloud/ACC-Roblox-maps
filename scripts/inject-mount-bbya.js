...
if(!String(target.file||'').startsWith('maps/mount-bbya/')) throw new Error('MOUNT BBYA FILE PATH LOCK MISMATCH');

const files={
 core:'maps/mount-bbya/mountain.phase1v6.server.lua',
 compose:'maps/mount-bbya/mountain.phase1v61.visual.server.lua',
 premium:'maps/mount-bbya/mount-bbya.phase1v65.visual.server.lua',
 signfix:'maps/mount-bbya/mount-bbya.sign-facing-hotfix.server.lua',
...
for(const f of Object.values(files)) if(!fs.existsSync(path.join(root,f))) throw new Error('Missing runtime donor: '+f);
...
workspace:SetAttribute('ACC_MountBBYA_SourceMode','V64_DONOR_PLUS_V65_PREMIUM')
workspace:SetAttribute('ACC_SpawnSafetyVersion','mount-bbya-v6.5')
...
local deadline=os.clock()+90;local r;local spawn
repeat r=workspace:FindFirstChild('ACC_MountainSocial');spawn=r and r:FindFirstChild('MountainSpawn');if r and spawn and r:GetAttribute('Phase1Ready')==true and r:GetAttribute('Phase1VisualReady')==true and r:GetAttribute('MountBBYAPhase1PremiumReady')==true then break end;if os.clock()>deadline then workspace:SetAttribute('ACC_SpawnGate','SAFE_HOLD_WORLD_TIMEOUT');warn('[MOUNT BBYA] readiness timeout');return end;task.wait(.2) until false
...
workspace:SetAttribute('ACC_ReleaseRuntimeOK',true);workspace:SetAttribute('ACC_MountainBuild','mount-bbya-v6.5-phase1-premium')`;
...
const qc=`task.delay(20,function()
 ...
 local ok=r~=nil and workspace:GetAttribute('ACC_BootstrapRuntimeOK')==true and workspace:GetAttribute('ACC_ReleaseRuntimeOK')==true and workspace:GetAttribute('ACC_SpawnGate')=='READY' and r:GetAttribute('Phase1Scope')=='SPAWN_TO_CP1_ONLY' and r:GetAttribute('TerrainFrozen')==true and r:GetAttribute('Phase1Ready')==true and r:GetAttribute('Phase1VisualReady')==true and r:GetAttribute('MountBBYAPhase1PremiumReady')==true and r:GetAttribute('MountBBYASignFacingReady')==true and cp1==true
 workspace:SetAttribute('ACC_MountBBYA_Phase1QC',ok and 'PASS' or 'FAIL');workspace:SetAttribute('ACC_MountainReady',ok)
 if not ok then warn('[MOUNT BBYA] Phase1 runtime QC failed')end
end)`;

const gate=(src,attr)=>`local deadline=os.clock()+80;repeat local r=workspace:FindFirstChild('ACC_MountainSocial');if r and r:GetAttribute('${attr}')==true then break end;if os.clock()>deadline then error('gate timeout: ${attr}')end;task.wait(.2)until false\n${src}`;
const checkpoint=gate(read(files.checkpoint),'Phase1VisualReady');
const premium=gate(read(files.premium),'Phase1VisualReady');
const releaseGated=`local deadline=os.clock()+85;repeat local r=workspace:FindFirstChild('ACC_MountainSocial');if r and r:GetAttribute('MountBBYASignFacingReady')==true then break end;if os.clock()>deadline then error('sign-facing gate timeout')end;task.wait(.2)until false\n${release}`;
...
const servers=[item('Script','BOOT','MountBBYA_Bootstrap',bootstrap),item('Script','CORE','MountBBYA_Phase1_Core',read(files.core)),item('Script','VIS61','MountBBYA_Phase1_Visual61',read(files.compose)),item('Script','VIS65','MountBBYA_Phase1_Premium65',premium),item('Script','SIGNFIX','MountBBYA_SignFacingFix',read(files.signfix)),item('Script','AMB','MountBBYA_Ambience',read(files.ambience)),item('Script','CP','MountBBYA_Checkpoint',checkpoint),item('Script','REL','MountBBYA_Release',releaseGated),item('Script','QC','MountBBYA_QC',qc)].join('');
...
console.log('[MOUNT BBYA] candidate built',target.file,Buffer.byteLength(xml),'bytes');
