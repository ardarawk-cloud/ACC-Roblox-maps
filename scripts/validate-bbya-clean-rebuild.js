...
const [place,core,architecture,exterior,furnishing,interior,circulation,lighting,runtime,social,qc,releaseGate,ui,assembler,previewInspector]=files.map(read);
...
if(!architecture.includes('STAIR G TO MID",Vector3.new(94,1.1,72),28,8,.55,.75,0')||!architecture.includes('STAIR MID TO ROOF",Vector3.new(88,16.7,47),28,8,.55,.75,180')) bad('switchback stair directions are not locked to corrected circulation');
else ok('reference architecture and corrected switchback circulation present');
...
if(!exterior.includes('LEFT_CLUB_CENTER_SOCIAL_RIGHT_VIP_UPPER_POOL')) bad('owner-reference hero silhouette marker missing'); else ok('premium exterior preserves owner-reference hero silhouette');
...
if(!fail) ok('premium social/hospitality furnishing and interior present');
...
if(!circulation.includes('BBYAClearLane')||!circulation.includes('BBYACirculationLight')) bad('clear-lane/light attributes missing'); else ok('circulation lanes, landing safety and route lighting present');
...
if(!ui.includes('supportLaunch')||!ui.includes('musicLaunch')||!ui.includes('photoLaunch')||!ui.includes('mini.MouseButton1Click')||!ui.includes('panel.Position=UDim2.fromOffset(x,y)')) bad('mobile launcher/mini-player/recovery layout incomplete'); else ok('reference-fidelity Support/Music/Photo mobile shell present');
...
for(const token of ['PHASE_6_PREVIEW_GATE','BBYAReleaseGate','BBYAReleaseBlockerCount','BBYAReleasePendingExternalCount','BBYAOwnerPlaytestRequired','RUNTIME_QC_PASS_PENDING_EXTERNAL_IDS_AND_OWNER_PLAYTEST','BBYAPhase5QC','BBYASupportTierCount','BBYAMusicDeckCount']) if(!releaseGate.includes(token)) bad(`phase 6 release-gate token missing: ${token}`);
if(!releaseGate.includes('task.delay(10')||!releaseGate.includes('state="BLOCKED"')) bad('phase 6 release gate does not wait for QC or block failures'); else ok('phase 6 read-only runtime release gate present');
...
for(const f of files.slice(1,12)) if(!assembler.includes(f)) bad(`assembler missing server module ${f}`);
if(!assembler.includes('maps/a-club/rebuild/70-ui.client.lua')) bad('assembler missing client UI module');
...
const order=['00-core.lua','10-architecture.lua','15-premium-exterior.lua','20-furnishing.lua','25-premium-interior.lua','35-circulation.lua','30-lighting.lua','40-runtime.server.lua','60-social-systems.server.lua','50-qc.server.lua','80-release-gate.server.lua'];
let last=-1;
for(const token of order){const i=assembler.indexOf(token);if(i<=last) bad(`server assembler order invalid at ${token}`);last=i;}
if(!fail) ok('phase 6 server assembly order locked');

for(const token of ['PHASE_6_PREVIEW_GATE','BBYAReleaseGate','SUPPORT_ORDER={5,10,50,100,500}','MUSIC_LIBRARY={club={},rooftop={}}','expected exactly one assembled server Script','preview unexpectedly contains live Developer Product IDs']) if(!previewInspector.includes(token)) bad(`preview inspector token missing: ${token}`);
if(!previewInspector.includes('process.exit(1)')) bad('preview inspector cannot fail CI'); else ok('assembled RBXLX preview inspector present');

if(fail) process.exit(1);
ok('BBYA clean rebuild phase 6 static gate complete');
