...
REPORT=ROOT/'deploy-status'/f'bbya-underground-track{TRACK_INDEX:03d}.json'
...
EXPECTED_USER_ID=os.environ.get('EXPECTED_AUDIO_OWNER_USER_ID','').strip()
...
def create_asset(title,creator):
    payload={
        ...
        'description':f'BBYA Underground Track {TRACK_INDEX:03d}',
...
def wait_mod(aid,state):
    ...
    while not approved(state) and not rejected(state) and time.time()<end:
        c,d=fetch_asset(aid); n=modval(d); state=n or state
        hist.append({'http':c,'state':state})
...
def finish(reg,track,report,status,blacklist=False):
    track['status']=status
    report['status']=status
    if blacklist:
        track['blacklisted']=True
        track['blacklistReason']='ROBLOX_MODERATION_REJECTED'
        track['blacklistedAtUnix']=int(time.time())
...
def main():
    ...
    report={
        'status':'STARTED','readyToInject':False,'playlistId':reg.get('playlistId'),
        'trackIndex':TRACK_INDEX,'title':track.get('title'),'driveFileId':track.get('driveFileId'),
        'bbyaUniverseId':BBYA_UNIVERSE,'assetId':track.get('assetId'),'uploadReused':bool(track.get('assetId')),
        'expectedUploaderUserId':EXPECTED_USER_ID or reg.get('expectedUploaderUserId')
    }
    if not AUDIO_KEY: finish(reg,track,report,'SECRET_MISSING')
    key=introspect(); report['key']=key
    expected=EXPECTED_USER_ID or str(reg.get('expectedUploaderUserId') or '')
    if key['http']!=200 or not key['authorizedUserId'] or not key['assetReadWrite'] or not key['assetPermissionsWrite'] or key['enabled'] is False or key['expired'] is True:
        finish(reg,track,report,'KEY_SCOPE_FAILED')
    if expected and str(key['authorizedUserId'])!=str(expected):
        finish(reg,track,report,'UPLOADER_ID_MISMATCH')
    ...
    if not aid:
        if not AUDIO_PATH.exists() or AUDIO_PATH.stat().st_size<10000:
            finish(reg,track,report,'AUDIO_FILE_MISSING')
        c,d=create_asset(track['title'],key['authorizedUserId']); report['uploadHttp']=c; report['uploadResponse']=d
        if c not in (200,201,202) or not d.get('path'):
            finish(reg,track,report,'UPLOAD_FAILED')
        op=poll(d['path']); report['operationResult']=op; r=(op or {}).get('response') or {}
        aid=str(r.get('assetId') or ''); state=modval(r)
        if not aid: finish(reg,track,report,'ASSET_ID_MISSING')
        track['assetId']=aid; track['moderationLastKnown']=state or None; track['status']='UPLOADED_MODERATION_PENDING'
        report['assetId']=aid; report['uploadReused']=False

    state,hist=wait_mod(aid,state); report['moderationState']=state; report['moderationChecks']=hist
    track['moderationLastKnown']=state or track.get('moderationLastKnown')
    if rejected(state):
        finish(reg,track,report,'BLACKLISTED_MODERATION_REJECTED',blacklist=True)
    if not approved(state):
        finish(reg,track,report,'MODERATION_PENDING')

    g=grant(aid); report['bbyaPermission']=g; track['bbyaPermission']=bool(g['ok'])
    if not g['ok']:
        finish(reg,track,report,'PERMISSION_FAILED')

    track['status']='READY_TO_INJECT'; report['status']='READY_TO_INJECT'; report['readyToInject']=True
    save(REGISTRY,reg); save(REPORT,report)
