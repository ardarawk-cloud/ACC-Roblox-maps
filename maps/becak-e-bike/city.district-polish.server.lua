-- BECAK E-BIKE district polish v1.1
-- Lightweight visual pass to reduce repeated box/blockout appearance across Nusakarya.
-- Adds district-specific storefront identity, roofline variation, recess framing and organic street furniture.
...
local function addStorefront(f,cf,size,front,pal,district,variant)
    ...
    -- Projecting awning breaks the vertical box face and changes by district.
    ...
    -- Recess frame gives the entry a visible depth layer without collidable geometry.
    ...
    -- Blade sign is thin and rotated; it reads as shop identity instead of another cube.
...
local function addRoofVariation(f,cf,size,pal,variant)
    ...
    if variant==1 then
        -- Offset dual wedge brows create an asymmetric pitched silhouette.
    ...
    elseif variant==2 then
        -- Small rounded rooftop pavilion gives civic/commercial blocks a softer crown.
        ...
        -- Pergola rhythm replaces a flat empty roofline with thin posts and beams.
...
local function decorate(model,body)
    ...
    -- Sparse vertical fins give taller slabs shadow/depth breaks instead of a flat face.
...
for i,data in ipairs(nodes) do
    ...
    -- Low curved-looking cluster using round forms, still visual-only.
    for n=1,3 do
