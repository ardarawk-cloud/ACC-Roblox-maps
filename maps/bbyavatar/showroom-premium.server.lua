-- BBYAVATAR premium showroom architecture v1.1
-- Self-contained so failures in feature scripts cannot blank the core showroom.
...
for _, x in ipairs({-40, -34, -28}) do
    ...
    mirror.Reflectance=0.14; mirror.CanCollide=false
...
for _,x in ipairs({-22,22}) do
    ...
    local cap=premiumPart("EntranceGlowCap",Vector3.new(5.1,0.22,5.1),CFrame.new(x,3.95,65),Enum.Material.Neon,Color3.fromRGB(101,115,170));cap.CanCollide=false
end
root:SetAttribute("VisualRevision","PREMIUM_ARCH_V1_1_ISOLATED")
...
print(string.format("[BBYAVATAR] Premium showroom v1.1 ready • %d parts • %d lights",premiumPartCount,dynamicLightCount))