-- BBYA Music Vault premium mobile panel v2.0
...
open.Name="MusicButton";open.Size=UDim2.fromOffset(50,50);open.AnchorPoint=Vector2.new(1,0);open.Position=UDim2.new(1,-18,0,58)
open.BackgroundColor3=BG;open.Text="♫";open.TextSize=24;open.TextColor3=PINK;open.Font=Enum.Font.GothamBlack;open.Parent=gui;corner(open,16);stroke(open,PINK,.35,1.2)

local frame=Instance.new("Frame")
frame.Name="Panel";frame.AnchorPoint=Vector2.new(1,0);frame.Position=UDim2.new(1,-16,0,116);frame.Size=UDim2.fromOffset(348,470)
...
title.Size=UDim2.new(1,-96,0,28);title.Position=UDim2.fromOffset(16,14);title.BackgroundTransparency=1;title.Text="BBYA MUSIC";title.TextColor3=WHITE;title.Font=Enum.Font.GothamBlack;title.TextSize=19;title.TextXAlignment=Enum.TextXAlignment.Left;title.Parent=frame
...
local bars={}
for i=1,18 do
 local b=Instance.new("Frame");b.AnchorPoint=Vector2.new(0,.5);b.Size=UDim2.fromOffset(5,math.random(4,14));b.Position=UDim2.new(0,(i-1)*7,.5,0);b.BorderSizePixel=0;b.BackgroundColor3=i%2==0 and CYAN or PINK;b.Parent=waveform;corner(b,2);bars[i]=b
end
...
local function ctl(txt,cb,w,accent)
 local b=Instance.new("TextButton");b.Size=UDim2.fromOffset(w or 48,44);b.BackgroundColor3=accent and Color3.fromRGB(55,26,62) or CARD;b.Text=txt;b.TextColor3=accent and PINK or WHITE;b.Font=Enum.Font.GothamBlack;b.TextSize=accent and 18 or 13;b.Parent=controls;corner(b,13);stroke(b,accent and PINK or Color3.fromRGB(88,76,102),accent and .45 or .75,1);b.Activated:Connect(cb);return b
end
...
local activeTab=nil
local function tab(txt,mode,w)
 local b=Instance.new("TextButton");b.Size=UDim2.fromOffset(w,36);b.BackgroundColor3=CARD;b.Text=txt;b.TextColor3=WHITE;b.Font=Enum.Font.GothamBold;b.TextSize=10;b.Parent=tabs;corner(b,10);b.Activated:Connect(function()control:FireServer("MODE",mode);if activeTab then activeTab.BackgroundColor3=CARD;activeTab.TextColor3=WHITE end;b.BackgroundColor3=Color3.fromRGB(56,26,64);b.TextColor3=PINK;activeTab=b end);return b
end
...
for _,sub in ipairs(subs)do
 local b=Instance.new("TextButton");b.BackgroundColor3=CARD2;b.Text=sub:gsub("_"," ");b.TextColor3=WHITE;b.Font=Enum.Font.GothamBold;b.TextSize=10;b.TextWrapped=true;b.Parent=scroll;corner(b,10);stroke(b,Color3.fromRGB(85,72,98),.8,1);b.Activated:Connect(function()control:FireServer("SUBGENRE",sub)end)
end

stateRemote.OnClientEvent:Connect(function(s)
 if typeof(s)~="table" then return end
 nowTitle.Text=tostring(s.title or "BBYA 24/7")
 local bits={}
 ...
 meta.TextColor3=s.error and s.error~="" and Color3.fromRGB(255,125,125) or MUTED
 if typeof(s.volume)=="number" then localVolume=s.volume;vol.Text="VOLUME "..math.floor(localVolume*100).."%" end
 live.Text=s.playing and "● LIVE" or "● PAUSED";live.TextColor3=s.playing and PINK or GOLD
end)
...
print("[BBYA] Premium Music Panel v2.0 loaded")