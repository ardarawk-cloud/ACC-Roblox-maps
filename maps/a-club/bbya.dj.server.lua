-- BBYA resident DJ NPC / booth performer v1.1
...
-- Clear narrow legacy booth obstructions around the performer area.
-- Keep the actual DJ Booth base; only disable tall thin posts/walls intersecting its center/front.
for _,o in ipairs(workspace:GetDescendants()) do
 if o:IsA("BasePart") and o~=booth and not o:IsDescendantOf(workspace:FindFirstChild("BBYA Resident DJ")) then
  local lp=booth.CFrame:PointToObjectSpace(o.Position)
  local tall=o.Size.Y>=7
  local nearBooth=math.abs(lp.X)<7 and math.abs(lp.Z)<7 and math.abs(lp.Y)<9
  local ln=string.lower(o.Name)
  local obstruction=string.find(ln,"pillar") or string.find(ln,"column") or string.find(ln,"support") or string.find(ln,"post") or string.find(ln,"wall")
  if tall and nearBooth and obstruction then
...
local function p(name,size,cf,color,material)
 local x=Instance.new("Part");x.Name=name;x.Size=size;x.CFrame=cf;x.Anchored=true;x.CanCollide=false
 x.Color=color;x.Material=material or Enum.Material.SmoothPlastic;x.Parent=model;return x
end

-- DJ faces the dance floor: rotate 180 degrees from the booth's native orientation.
-- Move him slightly behind the deck so he is visibly operating it, not buried in geometry.
...
-- Deck uses the same corrected facing as the performer.
...
local tag=Instance.new("BillboardGui");tag.Size=UDim2.fromOffset(150,32);tag.StudsOffset=Vector3.new(0,4.7,0);tag.MaxDistance=55;tag.AlwaysOnTop=false;tag.Parent=head
...
print("[BBYA DJ] Resident DJ orientation + booth clearance fixed")