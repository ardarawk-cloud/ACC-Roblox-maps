...
-- Hero composition lock: left multi-level club, center social atrium, right VIP wing, upper rooftop pool.
-- Priority: readable silhouette from arrival, real playable levels, no decorative fake floors.
...
part(A1,"CENTER APPROACH",Vector3.new(32,.25,58),CFrame.new(0,.72,-40),Color3.fromRGB(69,66,77),Enum.Material.Marble,0,true)
for _,x in ipairs({-64,-42,42,64}) do
    neon(A1,"ARRIVAL EDGE "..x,Vector3.new(16,.16,.16),CFrame.new(x,.75,-46),x<0 and C.pink or C.cyan)
end

-- =========================================================
-- LEFT CLUB WING — 3-TIER HERO MASS
-- =========================================================
part(A3,"CLUB GROUND SLAB",Vector3.new(104,2,105),CFrame.new(-34,0,35),Color3.fromRGB(28,27,36),Enum.Material.Slate,0,true)
part(A4,"MEZZ LEVEL 1",Vector3.new(38,1.4,84),CFrame.new(-76,16,42),C.charcoal,Enum.Material.Slate,0,true)
part(A4,"MEZZ LEVEL 2",Vector3.new(38,1.4,62),CFrame.new(-76,30,53),C.charcoal,Enum.Material.Slate,0,true)

-- Stepped tower frame gives a large stacked-club silhouette instead of a flat box.
for _,x in ipairs({-88,-76,-54}) do
    part(A4,"LEFT TOWER COLUMN "..x,Vector3.new(2,44,2),CFrame.new(x,22,30),C.graphite,Enum.Material.Metal,0,true)
end
for _,cfg in ipairs({
    {y=8,w=54},{y=16,w=54},{y=30,w=48},{y=43,w=42}
}) do
    part(A4,"LEFT TOWER BAND "..cfg.y,Vector3.new(cfg.w,1.2,2),CFrame.new(-70,cfg.y,-8),C.graphite,Enum.Material.Metal,0,true)
    neon(A4,"LEFT TOWER BAND NEON "..cfg.y,Vector3.new(cfg.w-6,.18,.18),CFrame.new(-70,cfg.y+.68,-9.05),cfg.y==30 and C.cyan or C.pink)
end
for _,cfg in ipairs({
    {y=8,h=13,w=50},{y=23,h=12,w=48},{y=36,h=10,w=42}
}) do
    local g=glass(A4,"LEFT GLASS FACADE "..cfg.y,Vector3.new(cfg.w,cfg.h,.5),CFrame.new(-70,cfg.y,-8.8),.44)
    g.CanCollide=false
end

-- Club enclosure keeps the main room dark and deep while the front remains visually open.
...
-- Central dance floor / integrated festival stage.
...
part(A3,"MAIN STAGE",Vector3.new(64,3,19),CFrame.new(-28,2.2,76),C.black,Enum.Material.Metal,0,true)
part(A3,"DJ BOOTH",Vector3.new(28,4.2,7),CFrame.new(-28,5.1,72.5),C.graphite,Enum.Material.Metal,0,true)
part(A3,"LED WALL",Vector3.new(70,14,.7),CFrame.new(-28,10.5,85.7),C.black,Enum.Material.SmoothPlastic,0,true)
part(A3,"STAGE CROWN HEADER",Vector3.new(78,2.5,4),CFrame.new(-28,19.5,84),C.graphite,Enum.Material.Metal,0,true)
neon(A3,"STAGE HEADER PINK",Vector3.new(66,.22,.22),CFrame.new(-28,18.3,81.9),C.pink)

-- =========================================================
-- CENTER SOCIAL ATRIUM — OPEN, BRIGHTER, LEGIBLE
-- =========================================================
...
-- Arrival portal stays visually lower than both club tower and rooftop, so BBYA silhouette reads in layers.
part(A2,"ATRIUM PORTAL BEAM",Vector3.new(58,2,5),CFrame.new(22,20,-8),C.charcoal,Enum.Material.Metal,0,true)
neon(A2,"ATRIUM PORTAL EDGE",Vector3.new(52,.18,.18),CFrame.new(22,18.9,-10.6),C.cyan)

-- =========================================================
-- RIGHT VIP WING — LUXURY TERRACE BASE FOR ROOFTOP
-- =========================================================
...
for _,x in ipairs({38,58,78,98}) do
    part(A5,"VIP FRONT COLUMN "..x,Vector3.new(1.1,18,1.1),CFrame.new(x,9,-7),C.graphite,Enum.Material.Metal,0,true)
end
for _,x in ipairs({48,68,88}) do
    local g=glass(A5,"VIP FRONT GLASS "..x,Vector3.new(17,13,.45),CFrame.new(x,7,-7.6),.40)
    g.CanCollide=false
end

-- =========================================================
-- ROOFTOP RESORT — PLAYABLE REAL LEVEL
-- =========================================================
...
-- Real infinity pool basin, shifted toward hero-facing front edge so water is visible from arrival.
...
-- Pool DJ is placed at the back, facing the pool/social deck.
part(A6,"POOL DJ ISLAND",Vector3.new(26,1.2,14),CFrame.new(48,32.3,66),C.graphite,Enum.Material.Slate,0,true)
part(A6,"POOL DJ DESK",Vector3.new(18,3.2,5),CFrame.new(48,34.4,66),C.black,Enum.Material.Metal,0,true)
neon(A6,"POOL DJ EDGE",Vector3.new(18,.18,.18),CFrame.new(48,35.9,63.4),C.pink)

-- =========================================================
-- REAL VERTICAL CIRCULATION
-- =========================================================
...
rail(A4,"MEZZ1 FRONT RAIL",Vector3.new(36,5,.45),CFrame.new(-76,19,-.5))
rail(A4,"MEZZ2 FRONT RAIL",Vector3.new(36,5,.45),CFrame.new(-76,33,22))

-- =========================================================
-- CENTRAL BBYA BRAND FACADE
-- =========================================================
-- Wordmark is integrated into the middle mass, not a detached billboard.
part(A2,"BRAND FACADE WALL",Vector3.new(92,15,2),CFrame.new(-8,28,-10),C.black,Enum.Material.Slate,0,true)
sign(A2,"MAIN BBYA WORDMARK","BBYA\nSOCIAL HUB",CFrame.new(-8,28,-11.1),Vector3.new(66,10,.35),C.pink,Enum.NormalId.Front)
neon(A2,"CROWN BASE",Vector3.new(24,.4,.4),CFrame.new(-8,35.3,-11.4),C.pink)
neon(A2,"CROWN L1",Vector3.new(9,.4,.4),CFrame.new(-17,38,-11.4)*CFrame.Angles(0,0,math.rad(58)),C.pink)
neon(A2,"CROWN L2",Vector3.new(9,.4,.4),CFrame.new(-11,39,-11.4)*CFrame.Angles(0,0,math.rad(-62)),C.pink)
neon(A2,"CROWN R2",Vector3.new(9,.4,.4),CFrame.new(-5,39,-11.4)*CFrame.Angles(0,0,math.rad(62)),C.pink)
...
workspace:SetAttribute("BBYAHeroComposition","LEFT_CLUB_CENTER_SOCIAL_RIGHT_VIP_UPPER_POOL")