-- BBYA Social Hub v1.5 supporter hall + functional feature stations
...
-- SUPPORTER HALL: persistent top supporters + Top 3 avatar portraits.
...
local slots={}
for i=1,3 do
 local slot=Instance.new("Frame");slot.Name="Top"..i;slot.Size=UDim2.new(1,0,.31,0);slot.Position=UDim2.new(0,0,(i-1)*.34,0);slot.BackgroundColor3=Color3.fromRGB(28,20,38);slot.BackgroundTransparency=.12;slot.Parent=podium;Instance.new("UICorner",slot).CornerRadius=UDim.new(0,10)
 local img=Instance.new("ImageLabel");img.Name="Avatar";img.Size=UDim2.fromOffset(72,72);img.Position=UDim2.fromOffset(8,6);img.BackgroundColor3=Color3.fromRGB(40,30,52);img.Parent=slot;Instance.new("UICorner",img).CornerRadius=UDim.new(1,0)
 local txt=Instance.new("TextLabel");txt.Name="Info";txt.BackgroundTransparency=1;txt.Size=UDim2.new(1,-92,1,-8);txt.Position=UDim2.fromOffset(88,4);txt.Text="#"..i.." —";txt.TextColor3=i==1 and Color3.fromRGB(255,220,90) or i==2 and Color3.fromRGB(210,220,240) or Color3.fromRGB(220,150,95);txt.Font=Enum.Font.GothamBold;txt.TextSize=18;txt.TextWrapped=true;txt.TextXAlignment=Enum.TextXAlignment.Left;txt.Parent=slot
 slots[i]={img=img,txt=txt}
end
local function refreshSupporters()
 local ok,pages=pcall(function()return supporterStore:GetSortedAsync(false,10)end)
 if not ok then rows.Text="Top Supporters\nLoading data...";return end
 local data=pages:GetCurrentPage();local lines={}
 for i=1,math.min(10,#data)do
  local entry=data[i];local uid=tonumber(string.match(tostring(entry.key),"(%d+)$")) or tonumber(entry.key);local name="User "..tostring(uid)
  if uid then pcall(function()name=Players:GetNameFromUserIdAsync(uid)end)end
  table.insert(lines,string.format("%d. %s  •  R$%d",i,name,entry.value or 0))
  if i<=3 then
   slots[i].txt.Text=string.format("#%d\n%s\nR$%d",i,name,entry.value or 0)
   if uid then local okThumb,url=pcall(function()return Players:GetUserThumbnailAsync(uid,Enum.ThumbnailType.HeadShot,Enum.ThumbnailSize.Size150x150)end);if okThumb then slots[i].img.Image=url end end
  end
 end
 rows.Text=#lines>0 and table.concat(lines,"\n") or "Waiting for supporters..."
 for i=#data+1,3 do if slots[i]then slots[i].txt.Text="#"..i.."\n—\nR$0";slots[i].img.Image="" end end
end
local lastTotals={}
local function hookPlayer(plr)
 lastTotals[plr.UserId]=plr:GetAttribute("TotalDonated")or 0
 plr:GetAttributeChangedSignal("TotalDonated"):Connect(function()
  local now=plr:GetAttribute("TotalDonated")or 0
  if now~=lastTotals[plr.UserId] then
   lastTotals[plr.UserId]=now
   pcall(function()supporterStore:SetAsync("u"..plr.UserId,now)end)
   task.defer(refreshSupporters)
  end
 end)
end
...
-- DONATE LIVE TEXT wall sits next to Top Supporters.
...
for _,plr in ipairs(Players:GetPlayers())do
 local before=plr:GetAttribute("TotalDonated")or 0
 plr:GetAttributeChangedSignal("TotalDonated"):Connect(function()local now=plr:GetAttribute("TotalDonated")or 0;if now>before then donateText.Text=string.format("THANK YOU %s!\n+R$%d • TOTAL R$%d",plr.DisplayName,now-before,now)end;before=now end)
end
Players.PlayerAdded:Connect(function(plr)local before=plr:GetAttribute("TotalDonated")or 0;plr:GetAttributeChangedSignal("TotalDonated"):Connect(function()local now=plr:GetAttribute("TotalDonated")or 0;if now>before then donateText.Text=string.format("THANK YOU %s!\n+R$%d • TOTAL R$%d",plr.DisplayName,now-before,now)end;before=now end)end)
...
for _,x in ipairs({-3,3})do local s=Instance.new("Seat");s.Name="Duo Photo Seat";s.Size=Vector3.new(2.6,1,2.6);s.CFrame=CFrame.new(69+x,1.7,-36)*CFrame.Angles(0,math.pi,0);s.Anchored=true;s.Color=Color3.fromRGB(80,45,95);s.Material=Enum.Material.Fabric;s.Parent=root end
...
print("[BBYA] v1.5 supporter hall loaded: persistent Top 10 + Top 3 avatar portraits + live donation wall")