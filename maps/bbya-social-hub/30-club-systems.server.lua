...
-- Main Club / VIP / Rooftop library: western/international only.
local PLAYLIST={
...
-- Main master feed owns the existing canonical names so legacy health guards remain compatible.
...
group.Volume=1
group:SetAttribute("BBYAAudioMode","MAIN_WESTERN_DUAL_DECK_V7")
group:SetAttribute("Venue","MAIN")
group:SetAttribute("GenrePolicy","WESTERN_INTERNATIONAL")
...
local eq=group:FindFirstChild("ClubEQ") or Instance.new("EqualizerSoundEffect")
eq.Name="ClubEQ";eq.LowGain=1.15;eq.MidGain=-.15;eq.HighGain=.45;eq.Parent=group
local compressor=group:FindFirstChild("VenueCompressor") or Instance.new("CompressorSoundEffect")
compressor.Name="VenueCompressor";compressor.Threshold=-10;compressor.Ratio=2.25;compressor.Attack=.06;compressor.Release=.28;compressor.GainMakeup=.5;compressor.Parent=group
...
local function stateData()
 ...
 return {
  ...
  queue=#requestQueue,audioMode="MAIN_WESTERN_AUTOMIX",venue="MAIN",genre="WESTERN",library=#PLAYLIST,
  liveDeck=deckName(activeDeck),standbyDeck=deckName(standbyDeck),standbyIndex=standbyIndex or 0,
...
local function quarantine(i,reason)
 ...
 warn(string.format("[BBYA/Main] quarantined %d %s (%s)",i,PLAYLIST[i] and PLAYLIST[i].title or "?",tostring(reason)))
 group:SetAttribute("LastBadTrack",i);group:SetAttribute("LastBadReason",tostring(reason));shuffleBag={}
end
...
local function desiredStandby()
 local req=firstValidRequest();if req then return req.index,true end
...
local function prepareStandby(i,fromRequest)
 ...
 standbyIndex=i;standbyFromRequest=fromRequest==true
 standbyDeck:Stop();standbyDeck.Volume=0;standbyDeck.SoundId=soundIdFor(i);standbyDeck.TimePosition=0
 standbyDeck:SetAttribute("PreparedIndex",i);standbyDeck:SetAttribute("PreparedReady",false);standbyDeck:SetAttribute("DeckRole","STANDBY")
 fireMusicState()
 task.spawn(function()
  ...
  if ok and waitLoaded(standbyDeck,LOAD_TIMEOUT) then
   standbyDeck:SetAttribute("PreparedReady",true);group:SetAttribute("StandbyReadyIndex",i)
  else
   quarantine(i,"preload_failed");standbyDeck:SetAttribute("PreparedReady",false);standbyIndex=nil;standbyFromRequest=false
   task.defer(function()local ni,nreq=desiredStandby();if ni then prepareStandby(ni,nreq) end end)
  end
  fireMusicState()
 end)
 return true
...
local function ensureDesiredStandby()
 ...
 local i,fromReq=desiredStandby();if not i then return end
 if standbyIndex~=i or standbyFromRequest~=(fromReq==true) then prepareStandby(i,fromReq) end
end
local function playOnDeckGuarded(deck,i,audible)
 ...
 local p0=deck.TimePosition;task.wait(.28)
 if deck.TimePosition<=p0+.02 then deck:Stop();quarantine(i,"timeline_stalled");return false end
 return true
end
...
local function startInitial()
 for _=1,#PLAYLIST+2 do
  local i=chooseStartup();if not i then break end
  if playOnDeckGuarded(activeDeck,i,true) then
   current=i;activeDeck:SetAttribute("DeckRole","LIVE");activeDeck:SetAttribute("PreparedIndex",i);activeDeck:SetAttribute("PreparedReady",true)
   standbyDeck:SetAttribute("DeckRole","STANDBY");shuffleBag={};fireMusicState();ensureDesiredStandby();return true
  end
 end
 return false
end
local function popRequestIfMatches(i)
 local req=requestQueue[1];if req and req.index==i then table.remove(requestQueue,1);return req end
end
local function transitionPrepared(forceImmediate)
 ...
 if not ready then
  ...
  while os.clock()<deadline and standbyIndex==nextIndex do if standbyDeck:GetAttribute("PreparedReady")==true then ready=true;break end;task.wait(.1) end
 end
 ...
 local wasRequest=standbyFromRequest;local req=wasRequest and popRequestIfMatches(nextIndex) or nil
 current=nextIndex;newDeck.TimePosition=0;newDeck.Volume=forceImmediate and 1 or 0
 newDeck:SetAttribute("DeckRole","MIXING_IN");oldDeck:SetAttribute("DeckRole","MIXING_OUT");newDeck:Play()
 local p0=newDeck.TimePosition;task.wait(.25)
 if not newDeck.IsPlaying or newDeck.TimePosition<=p0+.02 then
  newDeck:Stop();newDeck.Volume=0;oldDeck.Volume=1;oldDeck:SetAttribute("DeckRole","LIVE");quarantine(nextIndex,"incoming_stalled")
  transitioning=false;standbyIndex=nil;standbyFromRequest=false;if req then toastMain("Request dilewati: audio tidak dapat diputar.") end;ensureDesiredStandby();fireMusicState();return false
 end
 if req then toastMain("AutoMix request: "..PLAYLIST[nextIndex].title) end
 fireMusicState()
 if forceImmediate then oldDeck:Stop();oldDeck.Volume=0 else
  ...
  local down=TweenService:Create(oldDeck,ti,{Volume=0});local up=TweenService:Create(newDeck,ti,{Volume=1});down:Play();up:Play();up.Completed:Wait();oldDeck:Stop();oldDeck.Volume=0
 end
 activeDeck,standbyDeck=newDeck,oldDeck
 activeDeck:SetAttribute("DeckRole","LIVE");activeDeck:SetAttribute("PreparedIndex",current);activeDeck:SetAttribute("PreparedReady",true)
 standbyDeck:SetAttribute("DeckRole","STANDBY");standbyDeck:SetAttribute("PreparedIndex",0);standbyDeck:SetAttribute("PreparedReady",false);standbyDeck.SoundId=""
 standbyIndex=nil;standbyFromRequest=false;transitioning=false;ensureDesiredStandby();fireMusicState();return true
end
local function forcePlay(i)
 if not validTrack(i) then return false end
 transitioning=false;paused=false;activeDeck:Stop();standbyDeck:Stop();standbyDeck.Volume=0
 if playOnDeckGuarded(activeDeck,i,true) then current=i;standbyIndex=nil;standbyFromRequest=false;shuffleBag={};activeDeck:SetAttribute("DeckRole","LIVE");standbyDeck:SetAttribute("DeckRole","STANDBY");fireMusicState();ensureDesiredStandby();return true end
 return false
end
local function queueRequest(player,index)
 index=tonumber(index)
 if not player or not validTrack(index) then if player then stateRemote:FireClient(player,"toast","Track itu tidak tersedia.") end;return false end
 local now=os.clock();local last=requestCooldown[player.UserId] or 0
 if now-last<REQUEST_COOLDOWN then stateRemote:FireClient(player,"toast","Tunggu sebentar sebelum request lagi.");return false end
 ...
 requestCooldown[player.UserId]=now;table.insert(requestQueue,{playerId=player.UserId,index=index,requestedAt=os.time()})
 stateRemote:FireClient(player,"toast",string.format("Main Club queue #%d: %s • standby deck, tidak langsung nyala.",#requestQueue,PLAYLIST[index].title))
 stateRemote:FireClient(player,"djQueue",{position=#requestQueue,count=#requestQueue,title=PLAYLIST[index].title,now=PLAYLIST[current] and PLAYLIST[current].title or "",venue="MAIN"})
 ensureDesiredStandby();fireMusicState();return true
end

local function onDeckEnded(deck)
 if deck~=activeDeck or transitioning or paused then return end
 task.defer(function()if not transitionPrepared(true) then ensureDesiredStandby();task.wait(.35);transitionPrepared(true) end end)
end
deckA.Ended:Connect(function()onDeckEnded(deckA)end);deckB.Ended:Connect(function()onDeckEnded(deckB)end)

-- One public Music Remote, routed by the player's physical venue.
...
Players.PlayerAdded:Connect(function(player)
 task.delay(2,function()
  if player.Parent then
   ...
   if not isBasement(player) then stateRemote:FireClient(player,"playlist",PLAYLIST);fireMusicState(player) end
  end
 end)
end)
...
task.spawn(function()
 while task.wait(.20) do
  if not transitioning and not paused and activeDeck.IsPlaying then
   ...
   if len and len>5 then local remain=len-pos;if remain<=PRELOAD_WINDOW then ensureDesiredStandby() end;if remain<=MIX_SECONDS+.35 then task.spawn(function()transitionPrepared(false)end) end end
  end
 end
end)
