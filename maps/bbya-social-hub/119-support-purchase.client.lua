-- BBYA SOCIAL HUB — SUPPORT PURCHASE + DONATION NOTIFICATION v6.3 QC COMPACT LEFT
-- Native Roblox checkout remains the only purchase confirmation UI.
-- Runtime-QC correction only: keep the approved SFX, tighten the right-side empty space, and move the popup slightly left.
...
local gui=Instance.new("ScreenGui");gui.Name="BBYADonationNotificationUI";gui.ResetOnSpawn=false;gui.IgnoreGuiInset=true;gui.DisplayOrder=230;gui.ZIndexBehavior=Enum.ZIndexBehavior.Sibling;gui.Parent=pg;gui:SetAttribute("NotificationAuthority","NEW_DONATION_V6_3_QC_COMPACT_LEFT")
...
local POPUP_X=.485
local panel=Instance.new("Frame");panel.Name="NewDonationPopup";panel.AnchorPoint=Vector2.new(.5,0);panel.Position=UDim2.new(POPUP_X,0,.11,-110);panel.Size=UDim2.fromOffset(300,84);panel.BackgroundColor3=Color3.fromRGB(14,14,19);panel.BackgroundTransparency=.04;panel.BorderSizePixel=0;panel.Visible=false;panel.Parent=gui;corner(panel,13);stroke(panel,Color3.fromRGB(235,184,74),.22)
...
local amount=label(panel,"DONATED 0 R$",UDim2.fromOffset(88,42),UDim2.new(.54,-6,0,16),Enum.Font.GothamBold,9,Color3.fromRGB(247,55,158))
local total=label(panel,"TOTAL 0 R$",UDim2.new(.55,0,0,42),UDim2.new(.45,-10,0,16),Enum.Font.GothamBold,9,Color3.fromRGB(73,207,235))
...
local function showDonation(data)popupToken+=1;local token=popupToken;local donated=math.max(0,math.floor(tonumber(data.amount)or 0));local cumulative=math.max(donated,math.floor(tonumber(data.total)or donated));who.Text=string.upper(tostring(data.displayName or"SUPPORTER"));amount.Text="DONATED "..donated.." R$";total.Text="TOTAL "..cumulative.." R$";setAvatar(data.userId);panel.Visible=true;panel.Position=UDim2.new(POPUP_X,0,.11,-100);panel.BackgroundTransparency=.18;TweenService:Create(panel,TweenInfo.new(.22,Enum.EasingStyle.Quart,Enum.EasingDirection.Out),{Position=UDim2.new(POPUP_X,0,.11,0),BackgroundTransparency=.04}):Play();playDonationSfx();task.delay(4.6,function()if popupToken~=token then return end;local tw=TweenService:Create(panel,TweenInfo.new(.22),{Position=UDim2.new(POPUP_X,0,.11,-100)});tw:Play();tw.Completed:Wait();if popupToken==token then panel.Visible=false end end)end
...
print("[BBYA] Support v6.3 online: QC compact-left notification / approved SFX preserved")
