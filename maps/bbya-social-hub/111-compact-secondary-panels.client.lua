-- BBYA MUSIC UI TEST — MENU-ONLY COMPACT AUTHORITY v1
-- TEST TARGET ONLY: Universe 10762005984 / Place 124607344716828
-- IMPORTANT: this file ONLY owns the BBYA main feature menu geometry + scrolling.
-- It must NOT resize/reparent Dance, Carry, Support, Community, Message, Music, DJ or Party content.
...
local function applyMenu()
 ...
 drawer.Active=true
 if drawer:IsA("Frame") or drawer:IsA("ScrollingFrame") then
  drawer.BackgroundTransparency=math.max(drawer.BackgroundTransparency,.42)
 end
 drawer:SetAttribute("BBYAMainMenuAuthority","MENU_ONLY_COMPACT_SCROLL_V1")

 local host=drawer:FindFirstChild("BBYAMainMenuScrollOnlyV1")
 if not host then
  host=Instance.new("ScrollingFrame")
  host.Name="BBYAMainMenuScrollOnlyV1"
  host.Position=UDim2.fromOffset(10,72)
  host.Size=UDim2.new(1,-20,1,-84)
  host.BackgroundTransparency=1
  host.BorderSizePixel=0
  host.Active=true
  host.ScrollingEnabled=true
 ...
 for _,d in ipairs(drawer:GetDescendants()) do
  if isSlot(d) and d.Parent~=host then
   sourceParents[d.Parent]=true
   d.Parent=host
  end
 end
  ...
  if isSlot(d) then
   ...
   for _,x in ipairs(d:GetDescendants()) do
    if x:IsA("TextButton") then
     x.Active=true
     x.Selectable=true
     x.ZIndex=math.max(x.ZIndex,252)
 ...
 for p in pairs(sourceParents) do
  if p and p.Parent and p:IsA("GuiObject") and p~=drawer and p~=host then
   p.Visible=false
  end
 end

 local party=drawer:FindFirstChild("PartyStuffPanel",true)
 host.Visible=not (party and party:IsA("GuiObject") and party.Visible)

 local layout=host:FindFirstChild("CompactMenuListV1")
 ...
 if not drawer:GetAttribute("BBYAMainMenuBoundV1") then
  drawer:SetAttribute("BBYAMainMenuBoundV1",true)
  drawer:GetPropertyChangedSignal("Visible"):Connect(function()
   if drawer.Visible then task.defer(applyMenu);task.delay(.06,applyMenu) end
  end)
  drawer.DescendantAdded:Connect(function(d)
   if isSlot(d) then task.defer(applyMenu) end
  end)
  if party and party:IsA("GuiObject") then
   party:GetPropertyChangedSignal("Visible"):Connect(function()task.defer(applyMenu)end)
  end
 end
end

pg.ChildAdded:Connect(function(child)
 if child.Name=="BBYACommandMenuUI" then task.defer(applyMenu);task.delay(.2,applyMenu) end
end)
...
task.defer(applyMenu)
print("[BBYA TEST] Menu-only compact scroll authority v1 online")
