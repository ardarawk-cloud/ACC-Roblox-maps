-- BBYA MUSIC UI TEST — MENU-ONLY COMPACT AUTHORITY v2
...
local function applyMenu()
 ...
 local w,h=menuSize()
 drawer.AnchorPoint=Vector2.new(1,.5);drawer.Position=UDim2.new(1,-12,.5,0);drawer.Size=UDim2.fromOffset(w,h)
 ...
 drawer:SetAttribute("BBYAMainMenuAuthority","MENU_ONLY_COMPACT_SCROLL_V2")

 local host=drawer:FindFirstChild("BBYAMainMenuScrollOnlyV2") or drawer:FindFirstChild("BBYAMainMenuScrollOnlyV1")
 if host and host.Name~="BBYAMainMenuScrollOnlyV2" then host.Name="BBYAMainMenuScrollOnlyV2" end
 if not host then
  host=Instance.new("ScrollingFrame");host.Name="BBYAMainMenuScrollOnlyV2";host.Position=UDim2.fromOffset(10,72);host.Size=UDim2.new(1,-20,1,-84);host.BackgroundTransparency=1;host.BorderSizePixel=0;host.Active=true;host.ScrollingEnabled=true;host.ScrollingDirection=Enum.ScrollingDirection.Y;host.ElasticBehavior=Enum.ElasticBehavior.WhenScrollable;host.ScrollBarThickness=4;host.ScrollBarImageColor3=Color3.fromRGB(244,48,149);host.AutomaticCanvasSize=Enum.AutomaticSize.None;host.CanvasSize=UDim2.new();host.ClipsDescendants=true;host.ZIndex=250;host.Parent=drawer
  local layout=Instance.new("UIListLayout");layout.Name="CompactMenuListV2";layout.FillDirection=Enum.FillDirection.Vertical;layout.HorizontalAlignment=Enum.HorizontalAlignment.Center;layout.SortOrder=Enum.SortOrder.LayoutOrder;layout.Padding=UDim.new(0,8);layout.Parent=host
 ...
 local layout=host:FindFirstChildWhichIsA("UIListLayout");if layout then layout.Name="CompactMenuListV2" end

 local sourceParents={}
 for _,d in ipairs(drawer:GetDescendants()) do if isSlot(d) and d.Parent~=host then sourceParents[d.Parent]=true;d.Parent=host end end
 for _,d in ipairs(host:GetChildren()) do
  if isSlot(d) then
   ...
   for _,x in ipairs(d:GetDescendants()) do if x:IsA("TextButton") then x.Active=true;x.Selectable=true;x.ZIndex=math.max(x.ZIndex,252) elseif x:IsA("TextLabel") then x.ZIndex=math.max(x.ZIndex,252) end end
  end
 end
 ...
 if not drawer:GetAttribute("BBYAMainMenuBoundV2") then
  drawer:SetAttribute("BBYAMainMenuBoundV2",true)
...
pg.ChildAdded:Connect(function(child)if child.Name=="BBYACommandMenuUI" then task.defer(applyMenu);task.delay(.2,applyMenu) end end)
...
print("[BBYA TEST] Menu-only compact scroll authority v2 online")
