-- BBYA MUSIC UI TEST — CLUB GEAR v6 DETERMINISTIC HAND GRIP
-- TEST TARGET ONLY: Universe 10762005984 / Place 124607344716828
-- One server authority for Party Stuff. Tool remains usable, but visible grip is owned here.
...
local function baseTool(name,size,color,material)
 ...
 tool.Name=name; tool.RequiresHandle=true; tool.CanBeDropped=false; tool.ManualActivationOnly=false; tool.Enabled=true
 tool.ToolTip="BBYA cosmetic club prop"
 local h=Instance.new("Part")
 h.Name="Handle"; h.Size=size; h.Color=color; h.Material=material; h.CanCollide=false; h.CanTouch=false; h.CanQuery=false
...
local function makeGlowstick()
 local tool,h=baseTool("Glowstick",Vector3.new(.28,2.8,.28),COLORS[1],Enum.Material.Neon)
...
local function makeSparkler()
 local tool,h=baseTool("Party Sparkler",Vector3.new(.22,2.6,.22),Color3.fromRGB(80,80,84),Enum.Material.Metal)
 local att=Instance.new("Attachment"); att.Position=Vector3.new(0,1.28,0); att.Parent=h
 local emitter=Instance.new("ParticleEmitter")
 emitter.Enabled=false; emitter.Rate=42; emitter.Lifetime=NumberRange.new(.35,.7); emitter.Speed=NumberRange.new(2.5,5)
 emitter.SpreadAngle=Vector2.new(180,180); emitter.LightEmission=.9
 emitter.Size=NumberSequence.new({NumberSequenceKeypoint.new(0,.16),NumberSequenceKeypoint.new(1,0)})
...
local function makeGear(name)
 if name=="Money Gun" then return makeMoneyGun()
 elseif name=="Glowstick" then return makeGlowstick()
 elseif name=="Party Sparkler" then return makeSparkler() end
end

local function putAway(player)
 clearGear(player:FindFirstChildOfClass("Backpack"))
 clearGear(player.Character)
end
...
local function gripCFrame(name)
 if name=="Money Gun" then
  return CFrame.new(0,-.12,-.72)*CFrame.Angles(math.rad(-90),0,0)
 end
 return CFrame.new(0,-1.15,0)
end

local function forceRightGrip(char,tool,name)
 local hand=rightHand(char)
 local handle=tool and tool:FindFirstChild("Handle")
 if not hand or not hand:IsA("BasePart") or not handle or not handle:IsA("BasePart") then return false end

 local old=hand:FindFirstChild("RightGrip")
 if old then old:Destroy() end
 for _,j in ipairs(handle:GetChildren()) do
  if j.Name=="BBYAPartyGripFallback" then j:Destroy() end
 end

 local rel=gripCFrame(name)
 handle.CFrame=hand.CFrame*rel
 ...
 grip.C1=CFrame.new()
 grip.Parent=hand
 return grip.Part0==hand and grip.Part1==handle
end
...
local function equipGear(player,name)
 ...
 if not tool then result(player,false,name,"GEAR BUILD FAILED"); return end

 -- Let Roblox mark the Tool equipped, then replace the visual grip deterministically.
 ...
 task.delay(.12,function()
  ...
  local gripped=forceRightGrip(char,tool,name)
...
local function bindPlayer(player)
 player.CharacterAdded:Connect(function()
  task.defer(function() putAway(player); player:SetAttribute("BBYAPartyGear","") end)
 end)
 task.defer(function() putAway(player) end)
end
Players.PlayerAdded:Connect(bindPlayer)
for _,p in ipairs(Players:GetPlayers()) do bindPlayer(p) end
print("[BBYA TEST] Club Gear v6 online: deterministic RightGrip + no preload")
