...
local VENUE_NAME={MAIN="CLUB",UNDERGROUND="UNDERGROUND",VIP="VIP",FUNKOT="FUNKOT",SKATEPARK="SKATEPARK",ROOFTOP="ROOFTOP",NONE="MUSIC"}
local function currentVenue() return tostring(player:GetAttribute("BBYAAudioVenue") or "NONE") end
...
local function cleanLibrary(libraryCard)
 ...
 for _,child in ipairs(libraryCard:GetChildren()) do
  if child~=empty and not child:IsA("UICorner") and not child:IsA("UIStroke") and child:IsA("GuiObject") then child.Visible=false end
 end
 for _,d in ipairs(libraryCard:GetDescendants()) do
  if not d:IsDescendantOf(empty) and d:IsA("GuiObject") and d~=empty then d.Visible=false end
 end
 empty.Visible=true;empty.Position=UDim2.fromOffset(12,12);empty.Size=UDim2.new(1,-24,1,-24);empty.ZIndex=180
...
local function cleanPlayerCard(playerCard)
 ...
 for _,d in ipairs(playerCard:GetDescendants()) do
  if d:IsA("TextLabel") then
   ...
   if up:find("PROGRESSIVE",1,true) or up:find("BREAKBEAT",1,true) or up:find("INDO",1,true) or up:find("FUNKOT_ONLY",1,true) then d.Text=venue.." • 0 TRACKS" end
  end
 end
end
...
local function applyTransparency()
 for _,d in ipairs(panel:GetDescendants()) do
  if d:IsA("Frame") then if d.BackgroundTransparency<.28 then d.BackgroundTransparency=.28 end
  elseif d:IsA("ScrollingFrame") then if d.BackgroundTransparency<.32 then d.BackgroundTransparency=.32 end
  elseif d:IsA("TextButton") then if d.BackgroundTransparency<.16 then d.BackgroundTransparency=.16 end end
 end
...
local function placeEditorToggle()
 ...
 b.AnchorPoint=Vector2.new(0,1);b.Position=UDim2.new(0,16,1,-16);b.Size=UDim2.fromOffset(60,38);b.ZIndex=900
end

local function enforce()
 if not catalogResetActive() then return end
 local playerCard,libraryCard=findCore()
 cleanLibrary(libraryCard);cleanPlayerCard(playerCard);applyTransparency();placeEditorToggle()
 panel:SetAttribute("BBYAMusicResetUIClean",true);panel:SetAttribute("BBYALegacyPlaylistHidden",true)
end
...
local acc=0
RunService.Heartbeat:Connect(function(dt)acc+=dt;if acc<.15 then return end;acc=0;enforce() end)
