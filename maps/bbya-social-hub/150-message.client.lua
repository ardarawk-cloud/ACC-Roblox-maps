-- BBYA SOCIAL HUB — MESSAGE CLIENT v8.3 READABILITY
-- Composer/purchase flow preserved. Success popup keeps the existing visual family but gives
-- the actual message two readable lines and a longer 9.5s dwell time.
...
local gui=Instance.new("ScreenGui");gui.Name="BBYADJWallUI";gui.ResetOnSpawn=false;gui.IgnoreGuiInset=true;gui.DisplayOrder=940;gui.Parent=pg;gui:SetAttribute("BBYAUIAuthority","MESSAGE_V8_3_READABILITY")
...
local function showMessageSuccess(data)
 ...
 local popup=Instance.new("Frame");popup.Name="MessageSuccessPopup";popup.AnchorPoint=Vector2.new(.5,0);popup.Position=UDim2.new(.5,0,.10,-130);popup.Size=UDim2.fromOffset(350,118);popup.BackgroundColor3=Color3.fromRGB(14,14,19);popup.BackgroundTransparency=.04;popup.BorderSizePixel=0;popup.ZIndex=600;popup.Parent=gui;corner(popup,13);stroke(popup,C.pink,.22)
 local accent=Instance.new("Frame");accent.Size=UDim2.fromOffset(4,94);accent.Position=UDim2.fromOffset(9,12);accent.BackgroundColor3=C.pink;accent.BorderSizePixel=0;accent.ZIndex=601;accent.Parent=popup;corner(accent,3)
 local avatar=Instance.new("ImageLabel");avatar.Position=UDim2.fromOffset(22,18);avatar.Size=UDim2.fromOffset(60,60);avatar.BackgroundColor3=Color3.fromRGB(27,27,34);avatar.BorderSizePixel=0;avatar.ScaleType=Enum.ScaleType.Crop;avatar.ZIndex=601;avatar.Parent=popup;corner(avatar,30);stroke(avatar,C.pink,.18)
 ...
 local heading=label(popup,"MESSAGE SENT",UDim2.fromOffset(96,7),UDim2.new(1,-108,0,16),Enum.Font.GothamBlack,10,C.pink);heading.ZIndex=601
 local who=label(popup,string.upper(player.DisplayName),UDim2.fromOffset(96,23),UDim2.new(1,-108,0,18),Enum.Font.GothamBold,10,C.white);who.ZIndex=601
 ...
 local q=label(popup,"QUEUE #"..tostring(pos or 1),UDim2.fromOffset(96,41),UDim2.new(.48,-4,0,14),Enum.Font.GothamBold,8,C.cyan);q.ZIndex=601
 ...
 local preview=label(popup,textPreview,UDim2.fromOffset(96,60),UDim2.new(1,-110,0,47),Enum.Font.GothamBold,12,C.white);preview.ZIndex=601;preview.TextWrapped=true;preview.TextYAlignment=Enum.TextYAlignment.Top;preview.TextXAlignment=Enum.TextXAlignment.Left
 ...
 task.delay(POPUP_SECONDS,function()if notificationToken~=token or not popup.Parent then return end;local tw=TweenService:Create(popup,TweenInfo.new(.22),{Position=UDim2.new(.5,0,.10,-130)});tw:Play();tw.Completed:Wait();if popup.Parent and notificationToken==token then popup:Destroy()end end)
end
...
print("[BBYA] MESSAGE client v8.3 online: readable 2-line message / 9.5s popup / existing SFX + purchase flow")
