...
local snapshotRemote=remotes:WaitForChild("RolePanelSnapshot",25)
...
local panel=Instance.new("Frame");panel.Name="RolePanel";panel.AnchorPoint=Vector2.new(1,.5);panel.Position=UDim2.new(1,-18,.5,18);panel.Size=UDim2.fromOffset(420,420);panel.BackgroundColor3=C.bg;panel.BackgroundTransparency=.26;panel.BorderSizePixel=0;panel.Visible=false;panel.Parent=gui;corner(panel,16);stroke(panel,C.gold,.28)
...
local grid=Instance.new("Frame");grid.Position=UDim2.fromOffset(10,76);grid.Size=UDim2.new(1,-20,0,210);grid.BackgroundTransparency=1;grid.Parent=detail;local gl=Instance.new("UIGridLayout");gl.CellPadding=UDim2.fromOffset(6,6);gl.CellSize=UDim2.new(.5,-3,0,45);gl.FillDirectionMaxCells=2;gl.Parent=grid
...
local remove=button(detail,"REMOVE ROLE",UDim2.fromOffset(10,296),UDim2.new(1,-20,0,40),C.red);local status=text(detail,"Select an online player.",UDim2.fromOffset(12,338),UDim2.new(1,-24,0,18),Enum.Font.GothamBold,8,C.muted)
...
local function setSelection(e)selectedUserId=e and e.userId or nil;selectedLocked=e and e.locked==true or false;selectedName.Text=e and e.displayName or"SELECT PLAYER";selectedUser.Text=e and("@"..e.username)or"";currentRole.Text=e and("ROLE • "..e.role)or"ROLE —";currentRole.TextColor3=e and roleColor(e.role)or C.gold;for _,b in pairs(roleButtons)do b.Active=e~=nil and not selectedLocked;b.TextTransparency=b.Active and 0 or .5 end;remove.Active=e~=nil and not selectedLocked;remove.TextTransparency=remove.Active and 0 or .5;status.Text=selectedLocked and"OWNER role locked."or(e and"Choose a role."or"Select an online player.")end
local function clearRows()for _,c in ipairs(playerList:GetChildren())do if c:IsA("TextButton")then c:Destroy()end end end
local function render(snap)if type(snap)~="table"or snap.authorized~=true then return end;clearRows();local keep=nil;for i,e in ipairs(snap.players or{})do local row=button(playerList,"",nil,UDim2.new(1,-2,0,48),C.card);row.LayoutOrder=i;text(row,e.displayName,UDim2.fromOffset(8,5),UDim2.new(1,-16,0,18),Enum.Font.GothamBold,10,C.white);text(row,"@"..e.username,UDim2.fromOffset(8,24),UDim2.new(1,-16,0,15),Enum.Font.Gotham,7,C.muted);row.Activated:Connect(function()setSelection(e)end);if selectedUserId==e.userId then keep=e end end;if keep then setSelection(keep)elseif selectedUserId then setSelection(nil)end end
