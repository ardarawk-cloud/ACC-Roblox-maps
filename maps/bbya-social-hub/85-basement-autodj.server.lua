...
local function forBasementPlayers(fn)
 for _,p in ipairs(Players:GetPlayers()) do if isBasement(p) then fn(p) end end
end
local function toastBasement(msg)
 forBasementPlayers(function(p)stateRemote:FireClient(p,"toast",msg)end)
end
...
local group=SoundService:FindFirstChild("BBYABasementMaster")
...
group:SetAttribute("BBYAAudioMode","BASEMENT_INDO_DUAL_DECK_V1")
group:SetAttribute("Venue","BASEMENT")
group:SetAttribute("GenrePolicy","INDO_BREAKBEAT_FUNKOT_BOUNCE")
...
local function stateData()
 ...
 return {index=current,title=t and t.title or "",style=t and t.style or "",playing=activeDeck.IsPlaying and not paused,
  queue=#requestQueue,audioMode="BASEMENT_INDO_AUTOMIX",venue="BASEMENT",genre="INDO",library=#PLAYLIST,
...
local function quarantine(i,reason)
 ...
 warn(string.format("[BBYA/Basement] quarantined %d %s (%s)",i,PLAYLIST[i] and PLAYLIST[i].title or "?",tostring(reason)))
...
local function prepareStandby(i,fromRequest)
 ...
 standbyDeck:SetAttribute("PreparedIndex",i);standbyDeck:SetAttribute("PreparedReady",false);standbyDeck:SetAttribute("DeckRole","STANDBY");fireState()
 task.spawn(function()
...
local function playGuarded(deck,i,audible)
 ...
 local p0=deck.TimePosition;task.wait(.28);if deck.TimePosition<=p0+.02 then deck:Stop();quarantine(i,"timeline_stalled");return false end
 return true
end
...
local function startInitial()
 for _=1,#PLAYLIST+2 do local i=chooseStartup();if not i then break end;if playGuarded(activeDeck,i,true) then current=i;activeDeck:SetAttribute("DeckRole","LIVE");activeDeck:SetAttribute("PreparedIndex",i);activeDeck:SetAttribute("PreparedReady",true);standbyDeck:SetAttribute("DeckRole","STANDBY");shuffleBag={};fireState();ensureStandby();return true end end
 return false
end
...
local function transition(forceImmediate)
 ...
 transitioning=true;local oldDeck,newDeck=activeDeck,standbyDeck;local req=standbyFromRequest and popRequestIfMatches(nextIndex) or nil;current=nextIndex
 ...
 activeDeck,standbyDeck=newDeck,oldDeck;activeDeck:SetAttribute("DeckRole","LIVE");activeDeck:SetAttribute("PreparedIndex",current);activeDeck:SetAttribute("PreparedReady",true)
 standbyDeck:SetAttribute("DeckRole","STANDBY");standbyDeck:SetAttribute("PreparedIndex",0);standbyDeck:SetAttribute("PreparedReady",false);standbyDeck.SoundId="";standbyIndex=nil;standbyFromRequest=false;transitioning=false;ensureStandby();fireState();return true
end
local function forcePlay(i)
 if not validTrack(i) then return false end
 transitioning=false;paused=false;activeDeck:Stop();standbyDeck:Stop();standbyDeck.Volume=0
 if playGuarded(activeDeck,i,true) then current=i;standbyIndex=nil;standbyFromRequest=false;shuffleBag={};activeDeck:SetAttribute("DeckRole","LIVE");standbyDeck:SetAttribute("DeckRole","STANDBY");fireState();ensureStandby();return true end
 return false
end
local function queueRequest(player,index)
 ...
 stateRemote:FireClient(player,"toast",string.format("Basement queue #%d: %s • standby deck, tidak langsung nyala.",#requestQueue,PLAYLIST[index].title))
 stateRemote:FireClient(player,"djQueue",{position=#requestQueue,count=#requestQueue,title=PLAYLIST[index].title,now=PLAYLIST[current] and PLAYLIST[current].title or "",venue="BASEMENT"});ensureStandby();fireState();return true
end
...
task.spawn(function()
 while task.wait(.20) do
  if not transitioning and not paused and activeDeck.IsPlaying then local len,pos=activeDeck.TimeLength,activeDeck.TimePosition;if len and len>5 then local remain=len-pos;if remain<=PRELOAD_WINDOW then ensureStandby() end;if remain<=MIX_SECONDS+.35 then task.spawn(function()transition(false)end) end end end
 end
end)

task.spawn(function()
 task.wait(1.4)
 if not startInitial() then warn("[BBYA/Basement] No playable Indo track at startup") end
 local deadFor=0
 while task.wait(2.5) do
  if paused or activeDeck.IsPlaying then deadFor=0 else deadFor+=2.5;if deadFor>=5 then local i=chooseStartup();if i then forcePlay(i) end;deadFor=0 end end
 end
end)
