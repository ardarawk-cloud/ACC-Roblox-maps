-- BBYA SOCIAL HUB — DJ LIVE CLEAN UI v2.1
-- Compact control surface. BBYA MENU is the only entry point.
-- Access: managed DJ role + OWNER (@nadmo97) explicit QA exception.
...
gui:SetAttribute("RoleRequired","DJ_OR_OWNER_QA")
...
root:SetAttribute("BBYAOuterLayoutAuthority","DJ_LIVE_CLEAN_V2_1")
...
local function isOwnerQA()return string.lower(player.Name)==OWNER_USERNAME end
...
root:GetPropertyChangedSignal("Visible"):Connect(function()
 if root.Visible then
  if not accessAllowed() then root.Visible=false;return end
...
print("[BBYA] DJ LIVE CLEAN UI v2.1 online: MENU-only launcher / strict DJ + OWNER QA / compact controls / FX PAD")