-- BBYA SOCIAL HUB — DJ LIVE CLEAN UI v2
-- DJ-role-only compact control surface. No floating launcher; BBYA MENU is the only entry point.
...
gui:SetAttribute("BBYAFeature","DJ_LIVE")
gui:SetAttribute("LauncherRetired",true)
gui:SetAttribute("RoleRequired","DJ")
...
local function label(parent,value,pos,size,font,ts,color,align)
 local l=Instance.new("TextLabel");l.BackgroundTransparency=1;l.Text=tostring(value or "");l.Position=pos or UDim2.new();l.Size=size or UDim2.new();l.Font=font or Enum.Font.Gotham;l.TextSize=ts or 11;l.TextColor3=color or C.WHITE;l.TextXAlignment=align or Enum.TextXAlignment.Left;l.TextYAlignment=Enum.TextYAlignment.Center;l.TextTruncate=Enum.TextTruncate.AtEnd;l.Parent=parent;return l
end
local function button(parent,value,pos,size,color)
 local b=Instance.new("TextButton");b.Text=tostring(value or "");b.Position=pos or UDim2.new();b.Size=size or UDim2.new();b.BackgroundColor3=color or C.CARD;b.BorderSizePixel=0;b.TextColor3=C.WHITE;b.Font=Enum.Font.GothamBold;b.TextSize=10;b.AutoButtonColor=true;b.Parent=parent;corner(b,8);stroke(b,C.LINE,.45);return b
end
local function setActive(b,on)b.BackgroundColor3=on and C.ACTIVE or C.CARD;b.TextColor3=on and C.BLACK or C.WHITE end
local function frame(parent,pos,size,color,r)
 local f=Instance.new("Frame");f.Position=pos or UDim2.new();f.Size=size or UDim2.new();f.BackgroundColor3=color or C.PANEL;f.BorderSizePixel=0;f.Parent=parent;corner(f,r or 10);stroke(f,C.LINE,.35);return f
end

local root=Instance.new("Frame")
root.Name="DJLivePanel";root.Size=UDim2.fromScale(1,1);root.BackgroundColor3=C.BG;root.BorderSizePixel=0;root.Visible=false;root.Parent=gui
root:SetAttribute("BBYAOuterLayoutAuthority","DJ_LIVE_CLEAN_V2")
...
local status=label(header,"DJ ROLE • READY",UDim2.fromOffset(14,24),UDim2.fromOffset(260,16),Enum.Font.GothamBold,8,C.MUTED)
local close=button(header,"CLOSE",UDim2.new(1,-70,0,8),UDim2.fromOffset(60,32),C.RED);close.ZIndex=20
local live=button(header,"LIVE START",UDim2.new(1,-176,0,8),UDim2.fromOffset(98,32),C.CARD)
...
local function kernelMenuVisible(visible)
 local k=pg:FindFirstChild("BBYACommandMenuUI")
 local m=k and k:FindFirstChild("MenuButton",true)
 if m and m:IsA("GuiObject") then m.Visible=visible end
end
local function closePanel()
 root.Visible=false
 if dialog then dialog:Destroy();dialog=nil end
 kernelMenuVisible(true)
end
close.Activated:Connect(closePanel)
root:GetPropertyChangedSignal("Visible"):Connect(function()
 if root.Visible then
  if not hasDJRole() then root.Visible=false;return end
  kernelMenuVisible(false)
 else
  kernelMenuVisible(true)
 end
end)
...
local function waveform(parent)
 local h=Instance.new("Frame");h.BackgroundTransparency=1;h.ClipsDescendants=true;h.Size=UDim2.fromScale(1,1);h.Parent=parent
 for i=1,36 do
  local bar=Instance.new("Frame");bar.AnchorPoint=Vector2.new(.5,.5);bar.Position=UDim2.new((i-.5)/36,0,.5,0);bar.Size=UDim2.fromOffset(2,6+((i*13)%20));bar.BackgroundColor3=C.WHITE;bar.BackgroundTransparency=.52;bar.BorderSizePixel=0;bar.Parent=h
 end
end
...
local function makeDeck(deck,left)
 ...
 local badge=frame(f,UDim2.fromOffset(10,10),UDim2.fromOffset(34,34),C.CARD,7);local bl=label(badge,deck,UDim2.new(),UDim2.fromScale(1,1),Enum.Font.GothamBlack,16,C.WHITE,Enum.TextXAlignment.Center)
 local title=label(f,"EMPTY",UDim2.fromOffset(52,7),UDim2.new(1,-116,0,22),Enum.Font.GothamBold,13,C.WHITE)
 local artist=label(f,"",UDim2.fromOffset(52,28),UDim2.new(1,-116,0,16),Enum.Font.Gotham,9,C.MUTED)
 local bpm=label(f,"-- BPM",UDim2.new(1,-70,0,12),UDim2.fromOffset(60,20),Enum.Font.GothamBold,10,C.WHITE,Enum.TextXAlignment.Right)
 local wave=frame(f,UDim2.fromOffset(10,52),UDim2.new(1,-20,0,34),C.BLACK,7);wave.ClipsDescendants=true;waveform(wave)

 local cue=button(f,"CUE",UDim2.fromOffset(10,96),UDim2.fromOffset(58,38),C.CARD)
 local play=button(f,"PLAY",UDim2.fromOffset(74,96),UDim2.fromOffset(66,38),C.CARD)
 local sync=button(f,"SYNC",UDim2.fromOffset(146,96),UDim2.fromOffset(58,38),C.CARD)
 local fx=button(f,"FX",UDim2.new(1,-68,0,96),UDim2.fromOffset(58,38),C.ACTIVE);fx.TextColor3=C.BLACK

 local jog=Instance.new("Frame");jog.AnchorPoint=Vector2.new(.5,.5);jog.Position=UDim2.new(.5,0,.62,0);jog.Size=UDim2.fromOffset(94,94);jog.BackgroundColor3=C.BLACK;jog.BorderSizePixel=0;jog.Parent=f;corner(jog,47);local js=Instance.new("UIStroke");js.Color=C.LINE;js.Thickness=2;js.Parent=jog
 ...
 cue.Activated:Connect(function()action:FireServer("cue",{deck=deck})end)
 play.Activated:Connect(function()action:FireServer("play_toggle",{deck=deck})end)
 sync.Activated:Connect(function()action:FireServer("sync",{deck=deck})end)
 fx.Activated:Connect(function()gui:SetAttribute("OpenFXDeck",deck)end)
 playlist.Activated:Connect(function()gui:SetAttribute("OpenPlaylistDeck",deck)end)
end
...
local meterA,meterB={},{}
for i=1,12 do
 for side,tbl in ipairs({meterA,meterB}) do
  local b=Instance.new("Frame");b.AnchorPoint=Vector2.new(0,1);b.Position=UDim2.new(side==1 and 0 or .58,0,1,-(i-1)*8);b.Size=UDim2.new(.40,0,0,6);b.BackgroundColor3=C.WHITE;b.BackgroundTransparency=.75;b.BorderSizePixel=0;b.Parent=meterHolder;corner(b,2);tbl[i]=b
 end
end
...
local function setCross(inputX)
 local x=math.clamp((inputX-rail.AbsolutePosition.X)/math.max(1,rail.AbsoluteSize.X),0,1);thumb.Position=UDim2.new(x,0,.5,0);action:FireServer("crossfader",{value=x})
end
...
for i,name in ipairs(maps) do
 local b=button(footer,name,UDim2.new((i-1)/4,6,0,6),UDim2.new(.25,-12,0,34),C.CARD);b.TextSize=name=="UNDERGROUND" and 8 or 10;mapButtons[name]=b
 b.Activated:Connect(function()action:FireServer("map",{value=name})end)
end
...
local function modal(titleText)
 clearDialog();local shade=Instance.new("Frame");shade.Size=UDim2.fromScale(1,1);shade.BackgroundColor3=Color3.new(0,0,0);shade.BackgroundTransparency=.28;shade.ZIndex=100;shade.Parent=root
 ...
 label(box,titleText,UDim2.fromOffset(16,9),UDim2.new(1,-100,0,28),Enum.Font.GothamBlack,17,C.WHITE).ZIndex=102
 local x=button(box,"CLOSE",UDim2.new(1,-76,0,9),UDim2.fromOffset(62,30),C.CARD);x.ZIndex=103;x.Activated:Connect(clearDialog)
 dialog=shade;return box
end
local function openPlaylist(deck)
 if not hasDJRole() then return end
 local box=modal("PLAYLIST "..deck)
 local search=Instance.new("TextBox");search.Position=UDim2.fromOffset(16,48);search.Size=UDim2.new(1,-32,0,36);search.BackgroundColor3=C.CARD;search.TextColor3=C.WHITE;search.PlaceholderText="Search track...";search.PlaceholderColor3=C.MUTED;search.Text="";search.ClearTextOnFocus=false;search.Font=Enum.Font.Gotham;search.TextSize=11;search.BorderSizePixel=0;search.ZIndex=102;search.Parent=box;corner(search,8)
 ...
 local function render()
  ...
  for _,t in ipairs(library) do
   local map=t.map or "";local blob=string.lower(tostring(t.title or "").." "..tostring(t.artist or "").." "..map)
   if q=="" or string.find(blob,q,1,true) then
    local bpm=tonumber(t.bpm) or 0;local row=button(list,string.format("%s\n%s • %s%s",tostring(t.title),tostring(t.artist or ""),map,bpm>0 and (" • "..tostring(bpm).." BPM") or ""),nil,UDim2.new(1,-4,0,48),C.CARD);row.TextWrapped=true;row.TextXAlignment=Enum.TextXAlignment.Left;row.TextSize=10;row.ZIndex=103;row.Activated:Connect(function()action:FireServer("load",{deck=deck,index=t.index});clearDialog()end)
   end
  end
 end
...
local function openFx(deck)
 if not hasDJRole() then return end
 local box=modal("DECK "..deck.." • FX PAD")
 label(box,"LIVE FX",UDim2.fromOffset(16,48),UDim2.new(1,-32,0,18),Enum.Font.GothamBold,9,C.MUTED).ZIndex=102
 local fxNames={"ECHO","FILTER","REVERB","FLANGER"};local fxBtns={}
 for i,name in ipairs(fxNames) do
  local b=button(box,name.."\nOFF",UDim2.new((i-1)/4,16-(i-1)*4,0,72),UDim2.new(.25,-20,0,66),C.CARD);b.TextWrapped=true;b.TextSize=11;b.ZIndex=102;fxBtns[name]=b;b.Activated:Connect(function()action:FireServer("fx_toggle",{deck=deck,fx=name})end)
 end
 ...
 for i,name in ipairs(samples) do
  local col=(i-1)%2;local row=math.floor((i-1)/2);local b=button(box,name,UDim2.new(col*.5,16-col*4,0,180+row*62),UDim2.new(.5,-20,0,54),C.CARD);b.TextSize=12;b.ZIndex=102;b.Activated:Connect(function()action:FireServer("sample",{deck=deck,fx=name})end)
 end
 local function update()
  local d=state.decks and state.decks[deck] or {};for name,b in pairs(fxBtns) do local on=d.fx and d.fx[name]==true;b.Text=name.."\n"..(on and "ON" or "OFF");setActive(b,on) end
 end
...
local function applyState(s)
 ...
 state=s;status.Text=tostring(s.notice or (s.live and "DJ LIVE • "..tostring(s.map) or "DJ ROLE • READY"));live.Text=s.live and "LIVE • STOP" or "LIVE START";setActive(live,s.live==true)
 ...
 for deck,ref in pairs(deckRefs) do
  local d=s.decks and s.decks[deck] or {};ref.title.Text=tostring(d.title or "EMPTY");ref.artist.Text=tostring(d.artist or "");local bpm=tonumber(d.bpm) or 0;ref.bpm.Text=(bpm>0 and tostring(bpm) or "--").." BPM";ref.play.Text=d.playing and "PAUSE" or "PLAY";ref.jogStroke.Color=d.playing and C.WHITE or C.LINE
 end
...
local function refreshAuthorization()
 if not hasDJRole() then gui.Enabled=false;closePanel();return end
 gui.Enabled=true
...
player:GetAttributeChangedSignal("BBYAHasDJRole"):Connect(refreshAuthorization);player:GetAttributeChangedSignal("BBYAManagedRole"):Connect(refreshAuthorization)
...
print("[BBYA] DJ LIVE CLEAN UI v2 online: MENU-only launcher / compact mobile controls / FX PAD / strict DJ role")
