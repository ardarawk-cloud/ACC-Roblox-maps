-- BBYA SOCIAL HUB — VIP ENCLOSURE + SOUND SYNC v6
-- Closes all VIP walls except the front logo/entrance opening and guarantees working rear speakers.
...
local old=active:FindFirstChild("VIPEnclosureV6")
if old then old:Destroy() end
local out=Instance.new("Model");out.Name="VIPEnclosureV6";out.Parent=active
out:SetAttribute("ClosedWalls",true)
out:SetAttribute("FrontEntranceOpen",true)
out:SetAttribute("RearSoundGuaranteed",true)
...
-- Fully close rear and side walls. Front/south keeps a 30-stud opening for the entrance/logo sightline.
...
-- Acoustic/decorative vertical panels break up the solid walls without creating holes.
...
for _,x in ipairs({-48,-32,-16,0,16,32,48}) do
 part("RearPanel"..x,Vector3.new(10,12,.25),CFrame.new(x,34,43.65),C.panel,Enum.Material.Fabric,false,panels)
end

-- Ensure all four suspended arrays exist; specifically guarantee both rear/north corners.
...
local function ensureCluster(name,pos)
 ...
 local cf=CFrame.lookAt(pos,Vector3.new(target.X,pos.Y-1,target.Z))
 local cabinet=part("Cabinet",Vector3.new(5.2,7,3),cf,C.speaker,Enum.Material.Metal,false,cluster)
 cabinet:SetAttribute("Suspended",true);cabinet:SetAttribute("WallCorner",name)
 part("FrontGrille",Vector3.new(4.65,6.4,.12),cf*CFrame.new(0,0,-1.56),C.grille,Enum.Material.SmoothPlastic,false,cluster)
 for i,dy in ipairs({-1.55,1.2}) do
  local d=part("Driver"..i,Vector3.new(.18,2.35,2.35),cf*CFrame.new(0,dy,-1.64)*CFrame.Angles(0,math.rad(90),0),Color3.fromRGB(14,14,16),Enum.Material.SmoothPlastic,false,cluster)
  d.Shape=Enum.PartType.Cylinder
 end
 local right=cf.RightVector
 for i,offset in ipairs({-1.45,1.45}) do
  local anchor=pos+right*offset
  part("Hanger"..i,Vector3.new(.16,2.1,.16),CFrame.new(anchor.X,42.45,anchor.Z),C.metal,Enum.Material.Metal,false,cluster)
 end
 local s=Instance.new("Sound");s.Name="CornerSpatialAudio";s.Volume=.065;s.Looped=false;s.RollOffMode=Enum.RollOffMode.InverseTapered;s.RollOffMinDistance=10;s.RollOffMaxDistance=72;s.EmitterSize=14;s.Parent=cabinet
 return cluster
end
...
local function activeDeck()
 ...
 if a and a:IsA("Sound") and b and b:IsA("Sound") then
  ...
  if a.IsPlaying then return a end
  if b.IsPlaying then return b end
 elseif a and a:IsA("Sound") and a.IsPlaying then return a
 elseif b and b:IsA("Sound") and b.IsPlaying then return b end
end

-- Mirror the actual AutoDJ deck into the four positional arrays, keeping rear and front speakers in sync.
task.spawn(function()
  ...
  if deck and deck.SoundId~="" then
   for _,obj in ipairs(rig:GetDescendants()) do
    if obj:IsA("Sound") and obj.Name=="CornerSpatialAudio" then
     if obj.SoundId~=deck.SoundId then
      obj:Stop();obj.SoundId=deck.SoundId;obj.TimePosition=math.max(0,deck.TimePosition);obj:Play()
     elseif not obj.IsPlaying then
      obj.TimePosition=math.max(0,deck.TimePosition);obj:Play()
     elseif math.abs(obj.TimePosition-deck.TimePosition)>1.4 then
      obj.TimePosition=math.max(0,deck.TimePosition)
     end
     obj.PlaybackSpeed=deck.PlaybackSpeed
     obj.Volume=.065
    end
   end
  end
...
print("[BBYA] VIP v6 online: closed rear/sides + front logo opening + 4 synced corner speakers")
