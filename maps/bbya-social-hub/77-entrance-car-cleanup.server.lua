...
-- Canonical entrance-car runtime guard for two creator-permitted Ddiaz Design display cars.
-- The actual Roblox Model assets are baked into the published place by the dedicated packaging workflow.
-- This script MUST NOT load cloud Model assets at runtime and MUST NOT fabricate replacement cars.
...
local function normalizeAndPlace(model, x, yawDegrees)
	...
	-- Imported GLB orientation is preserved, then the pair receives only the intended valet yaw.
...
local pair = scene:WaitForChild(PAIR_NAME, 12)
local car01 = pair and pair:FindFirstChild("BBYA Car 01")
local car02 = pair and pair:FindFirstChild("BBYA Car 02")

if not pair or not car01 or not car02 or not car01:IsA("Model") or not car02:IsA("Model") then
	scene:SetAttribute("EntranceCarsQuarantined", true)
	scene:SetAttribute("EntranceCarAuthority", "BAKED_DDIAZ_CARS_V10_MISSING")
	scene:SetAttribute("FallbackCarsRetained", false)
	warn("[BBYA] baked Ddiaz pair missing; refusing runtime fallback fabrication")
	return
end

stripForDisplay(car01)
stripForDisplay(car02)
if countVisibleParts(car01) < 1 or countVisibleParts(car02) < 1 then
	scene:SetAttribute("EntranceCarsQuarantined", true)
	scene:SetAttribute("EntranceCarAuthority", "BAKED_DDIAZ_CARS_V10_INVALID")
	warn("[BBYA] baked Ddiaz pair contains no visible geometry")
	return
end
...
if not valid1 or not valid2 then
	...
	warn(string.format("[BBYA] baked car visual validation failed car01=%s car02=%s", tostring(geo1), tostring(geo2)))
	return
end

car01:SetAttribute("RobloxAssetId", CAR_01_ASSET_ID)
car01:SetAttribute("DisplayOnly", true)
car01:SetAttribute("Creator", "Ddiaz Design")
car01:SetAttribute("SourceUrl", "https://skfb.ly/pFsG9")
car01:SetAttribute("License", "CC BY-NC-SA 4.0")
car01:SetAttribute("CreatorPermission", true)
car01:SetAttribute("VisualValidation", geo1)
car02:SetAttribute("RobloxAssetId", CAR_02_ASSET_ID)
car02:SetAttribute("DisplayOnly", true)
car02:SetAttribute("Creator", "Ddiaz Design")
car02:SetAttribute("SourceUrl", "https://skfb.ly/pKzpM")
car02:SetAttribute("License", "CC BY-NC-SA 4.0")
car02:SetAttribute("CreatorPermission", true)
car02:SetAttribute("VisualValidation", geo2)
addDisplayCollision(car01)
addDisplayCollision(car02)

-- Retire every known old/fallback entrance-car authority only after the baked pair validates.
...
scene:SetAttribute("EntranceCarAuthority", "BAKED_DDIAZ_CARS_V10")
...
pair:SetAttribute("Pass", "BAKED_DDIAZ_CARS_V10")
pair:SetAttribute("TargetVehicleCount", 2)
pair:SetAttribute("CenterAccessKeptOpen", true)
pair:SetAttribute("DisplayOnly", true)
pair:SetAttribute("AttributionRequired", true)
