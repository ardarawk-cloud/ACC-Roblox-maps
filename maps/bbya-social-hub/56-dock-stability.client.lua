-- BBYA SOCIAL HUB — COMMAND MENU AUTHORITY v7
-- Single compact UI authority: one visible feature shell at a time.
-- MENU stays top-right; drawer + normal feature panels dock immediately LEFT of MENU.
-- Gameplay center and the bottom-right Roblox jump zone remain clear.
-- Critical consent modals (incoming carry request, etc.) are intentionally not moved.
...
for _,name in ipairs({"BBYAUnifiedMenuUI","BBYACommandMenuUI"}) do
...
local function stroke(o,c,tr)
 local x=o:FindFirstChild("CommandStroke") or Instance.new("UIStroke")
 x.Name="CommandStroke";x.Color=c or C.line;x.Thickness=1;x.Transparency=tr or .45;x.Parent=o;return x
end
local function label(parent,text,pos,size,font,ts,color,align)
 local l=Instance.new("TextLabel")
 l.BackgroundTransparency=1;l.Text=text;l.Position=pos;l.Size=size;l.Font=font or Enum.Font.Gotham;l.TextSize=ts or 10;l.TextColor3=color or C.white
...
menuGui:SetAttribute("BBYACommandMenuAuthority","V7_SINGLE_COMPACT_AUTHORITY")
menuGui:SetAttribute("BBYASinglePanelMode",true)
...
-- Right edge is 10px left of MENU's left edge: 12px screen margin + 74px button + 10px gap.
...
header.Position=UDim2.fromOffset(10,10);header.Size=UDim2.new(1,-20,0,52);header.BackgroundColor3=C.panel;header.BorderSizePixel=0;header.ZIndex=202;header.Parent=drawer;corner(header,11);stroke(header,C.line,.58)
...
body.Position=UDim2.fromOffset(10,70);body.Size=UDim2.new(1,-20,1,-80);body.BackgroundTransparency=1;body.ZIndex=202;body.Parent=drawer
local grid=Instance.new("UIGridLayout")
grid.CellPadding=UDim2.fromOffset(7,7);grid.CellSize=UDim2.new(.5,-4,0,40);grid.FillDirectionMaxCells=2;grid.SortOrder=Enum.SortOrder.LayoutOrder;grid.Parent=body
...
for _,spec in ipairs(specList) do
 ...
 slot.Name="Slot_"..spec[1];slot.LayoutOrder=spec[2];slot.BackgroundColor3=C.card;slot.BorderSizePixel=0;slot.ZIndex=202;slot.Parent=body;corner(slot,9);stroke(slot,spec[3],.68)
...
local observed={}
local panelBound={}
local visibilityBound={}
local scanAndAttach
local dockKnownPanels
local docking=false

local function panelRefs()
 ...
 local communityOverlay=clubUI:FindFirstChild("CommunityOverlay",true)
 local communityPanel=findNamed(communityOverlay,"CommunityPanel") or findNamed(clubUI,"CommunityPanel")
 local messagePanel=findNamed(wall,"DJWallComposerPanel")
 local roleShade=roleGui and roleGui:FindFirstChild("Shade",true)
 return {
  HUB={root=findNamed(clubUI,"HubPanel"),dock=findNamed(clubUI,"HubPanel")},
  MUSIC={root=musicLayer,dock=findNamed(musicLayer,"CompactMusicCardV7")},
  PLAYLIST={root=findNamed(musicLayer,"PlaylistDrawerV7"),dock=findNamed(musicLayer,"PlaylistDrawerV7")},
  COMMUNITY={root=communityOverlay or communityPanel,dock=communityPanel},
  DANCE={root=findNamed(social,"DancePanel"),dock=findNamed(social,"DancePanel")},
  CARRY={root=findNamed(social,"CarryPanel"),dock=findNamed(social,"CarryPanel")},
  MESSAGE={root=messagePanel,dock=messagePanel},
  ROLE={root=roleShade,dock=findNamed(roleGui,"RolePanel")},
 }
end

local function setHidden(obj)
 if obj and obj:IsA("GuiObject") and obj.Visible then obj.Visible=false end
end

local function hideCompetingPanels(keep)
 local refs=panelRefs()
 for key,ref in pairs(refs) do
  if key~=keep and not (keep=="MUSIC" and key=="PLAYLIST") and not (keep=="PLAYLIST" and key=="MUSIC") then
   setHidden(ref.root)
  end
 end
end
...
local function dockPanel(panel)
 ...
 panel:SetAttribute("BBYAMenuSideDock","LEFT_OF_MENU_V2_PRECISE")
 docking=false
 if not panelBound[panel] then
  panelBound[panel]=true
  panel:GetPropertyChangedSignal("Visible"):Connect(function()
   if panel.Visible then task.defer(function()dockPanel(panel)end);task.delay(.03,function()dockPanel(panel)end) end
  end)
...
dockKnownPanels=function()
 local refs=panelRefs()
 for _,ref in pairs(refs) do dockPanel(ref.dock) end
end

local function bindSinglePanelVisibility()
 local refs=panelRefs()
 for key,ref in pairs(refs) do
  local root=ref.root
  if root and root:IsA("GuiObject") and not visibilityBound[root] then
   visibilityBound[root]=true
   root:GetPropertyChangedSignal("Visible"):Connect(function()
    if root.Visible then
     closeDrawer()
     hideCompetingPanels(key)
     task.defer(dockKnownPanels);task.delay(.03,dockKnownPanels)
    end
   end)
  end
 end
...
local function keepKeyFor(id)
 if id=="MUSIC" then return "MUSIC" end
 if id=="BBYA" or id=="SUPPORT" or id=="TRAVEL" then return "HUB" end
 if id=="COMMUNITY" then return "COMMUNITY" end
 if id=="DANCE" then return "DANCE" end
 if id=="CARRY" then return "CARRY" end
 if id=="MESSAGE" then return "MESSAGE" end
 return nil
end
...
local function enforceButton(id,btn)
 ...
 slot.button=btn;slot.placeholder.Visible=false
 ...
 rebinding=false
 if not btn:GetAttribute("BBYACommandMenuGuardV7") then
  btn:SetAttribute("BBYACommandMenuGuardV7",true)
  btn.Activated:Connect(function()
   closeDrawer()
   local keep=keepKeyFor(id)
   task.defer(function()
    hideCompetingPanels(keep)
    bindSinglePanelVisibility()
    dockKnownPanels()
   end)
...
local function observe(container)
 ...
 container.DescendantAdded:Connect(function(d)
  ...
  task.defer(bindSinglePanelVisibility);task.defer(dockKnownPanels)
 end)
end
...
scanAndAttach=function()
 ...
 for _,obj in ipairs(dock:GetChildren()) do
  if obj:IsA("TextButton") then
   local id=classifyDockButton(obj)
   if id then enforceButton(id,obj) end
  end
 end
 ...
 if social then
  ...
  for _,obj in ipairs(social:GetChildren()) do
   if obj:IsA("TextButton") then
    local t=string.upper(tostring(obj.Text or ""))
    if t=="DANCE" then enforceButton("DANCE",obj) elseif t=="CARRY" then enforceButton("CARRY",obj) end
   end
  end
 end
 ...
 if free then
  observe(free)
  local b=free:FindFirstChild("FreecamToggle")
  if b and b:IsA("TextButton") then enforceButton("FREECAM",b) end
 end
 observe(clubUI)
...
menuButton.Activated:Connect(function()
 if drawer.Visible then
  ...
  hideCompetingPanels(nil)
  drawer.Visible=true
  menuButton.Text="CLOSE"
  scanAndAttach();layout()
 end
end)
...
pg.ChildAdded:Connect(function(child)observe(child);task.defer(scanAndAttach);task.delay(.05,function()bindSinglePanelVisibility();dockKnownPanels()end) end)
...
task.defer(function()scanAndAttach();layout();bindSinglePanelVisibility();dockKnownPanels()end)
task.delay(.5,function()scanAndAttach();layout();bindSinglePanelVisibility();dockKnownPanels()end)

print("[BBYA] Command Menu v7 online: single compact panel authority / precise left-of-MENU dock / no double shells")
