...
-- Dedicated Mall-only 18-track KPOP bank. Never reuses Main Club catalog/state.
...
if not control then
    ...
    control.Name="BBYAMallMusicControl"
...
if not master then
    master=Instance.new("Sound")
    master.Name="BBYAMallMasterSound"
...
local function publishCatalog()
    ...
    if not folder then
        ...
        folder.Name="BBYAMallPlaylistCatalog"
...
local function loadCurrent()
    ...
    master:SetAttribute("PlaylistId","mall-kpop")
    master:SetAttribute("Bank","KPOP")
    publishState()
    pcall(function() master:Play() end)
end
...
control.OnServerEvent:Connect(function(player,action,wanted)
    ...
    if action=="next" then
        currentIndex=popNext(); loadCurrent(); bindEnded()
    elseif action=="prev" or action=="previous" then
        currentIndex=((currentIndex-2)%#PLAYLIST)+1; loadCurrent(); bindEnded()
    elseif action=="play" then
        local n=tonumber(wanted)
        if n and PLAYLIST[n] then currentIndex=n; loadCurrent(); bindEnded() end
...
task.spawn(function()
    while task.wait(0.5) do
        ...
        if math.abs(master.PlaybackSpeed-PLAYBACK_SPEED)>0.000001 then master.PlaybackSpeed=PLAYBACK_SPEED end
        if master.SoundId~=expected or master.SoundId=="" then
            loadCurrent(); bindEnded()
        elseif not master.IsPlaying then
            local atEnd=false
            pcall(function()
                atEnd=master.TimeLength>0 and master.TimePosition>=math.max(0,master.TimeLength-0.25)
            end)
            if atEnd then currentIndex=popNext(); loadCurrent(); bindEnded()
            else pcall(function() master:Play() end) end
        end
    end
end)
