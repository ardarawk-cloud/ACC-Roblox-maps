-- BBYA SOCIAL HUB — PREMIUM ENTRANCE CARS v1
-- Loads two BBYA-owned imported GLB model assets into the existing entrance slots.
-- Source geometry: Spectral GT RS (CC0), recolored deterministically by CI.
-- IMPORTANT: asset IDs remain zero until the ingest workflow successfully uploads both variants.
...
-- Let EntranceCarCleanup complete first. We intentionally never restore blocky fallbacks.
task.wait(1.0)
...
local function normalizeLength(model)
	...
	model:ScaleTo(currentScale * (TARGET_LENGTH / horizontal))
end
...
local function groundModel(model, targetCF)
	...
	local dy = ROAD_SURFACE_Y - bottomY
...
local function addDisplayCollision(model)
	...
	blocker.Size = Vector3.new(math.max(2, size.X * 0.82), math.max(1.4, size.Y * 0.52), math.max(2, size.Z * 0.82))
	blocker.CFrame = CFrame.new(boxCF.Position.X, ROAD_SURFACE_Y + blocker.Size.Y * 0.5, boxCF.Position.Z) * (model:GetPivot() - model:GetPivot().Position)
...
-- Parallel to the curb, facing toward the central arrival/crosswalk.
