...
local gui=Instance.new("ScreenGui");gui.Name="BBYAClubUI";gui.ResetOnSpawn=false;gui.IgnoreGuiInset=true;gui.DisplayOrder=10;gui.Parent=player:WaitForChild("PlayerGui")
...
local function btn(parent,text,pos,size,color)local b=Instance.new("TextButton");b.Text=text;b.Position=pos;b.Size=size;b.BackgroundColor3=color or CARD;b.TextColor3=Color3.fromRGB(244,244,247);b.Font=Enum.Font.GothamSemibold;b.TextSize=14;b.BorderSizePixel=0;b.Parent=parent;round(b,9);return b end
...
local subhead=Instance.new("TextLabel");subhead.BackgroundTransparency=1;subhead.Position=UDim2.fromOffset(0,26);subhead.Size=UDim2.new(1,-44,0,18);subhead.Text="HYBRID DJ  •  SUPPORT  •  TELEPORT";subhead.TextColor3=MUTED;subhead.Font=Enum.Font.GothamMedium;subhead.TextSize=9;subhead.TextXAlignment=Enum.TextXAlignment.Left;subhead.Parent=head
...
local musicTab=btn(tabs,"MUSIC",UDim2.fromOffset(4,4),UDim2.new(.333,-5,1,-8),Color3.fromRGB(74,22,55))
local supportTab=btn(tabs,"SUPPORT",UDim2.new(.333,2,0,4),UDim2.new(.333,-5,1,-8),Color3.fromRGB(26,25,31))
local teleportTab=btn(tabs,"TELEPORT",UDim2.new(.666,0,0,4),UDim2.new(.334,-4,1,-8),Color3.fromRGB(26,25,31));teleportTab.TextSize=12
...
local teleport=Instance.new("Frame");teleport.BackgroundTransparency=1;teleport.Size=UDim2.fromScale(1,1);teleport.Visible=false;teleport.Parent=content
local tpHead=Instance.new("TextLabel");tpHead.BackgroundTransparency=1;tpHead.Size=UDim2.new(1,0,0,34);tpHead.Text="QUICK TELEPORT";tpHead.TextColor3=PINK;tpHead.Font=Enum.Font.GothamBold;tpHead.TextSize=16;tpHead.TextXAlignment=Enum.TextXAlignment.Left;tpHead.Parent=teleport
local tpSub=Instance.new("TextLabel");tpSub.BackgroundTransparency=1;tpSub.Position=UDim2.fromOffset(0,32);tpSub.Size=UDim2.new(1,0,0,34);tpSub.Text="Pindah area tanpa tangga.";tpSub.TextColor3=MUTED;tpSub.Font=Enum.Font.Gotham;tpSub.TextSize=12;tpSub.TextXAlignment=Enum.TextXAlignment.Left;tpSub.Parent=teleport
local tpHolder=Instance.new("ScrollingFrame");tpHolder.BackgroundTransparency=1;tpHolder.BorderSizePixel=0;tpHolder.Position=UDim2.fromOffset(0,78);tpHolder.Size=UDim2.new(1,0,1,-78);tpHolder.ScrollBarThickness=3;tpHolder.AutomaticCanvasSize=Enum.AutomaticSize.Y;tpHolder.CanvasSize=UDim2.new();tpHolder.Parent=teleport
local tpGrid=Instance.new("UIGridLayout");tpGrid.CellSize=UDim2.new(.48,0,0,52);tpGrid.CellPadding=UDim2.new(.04,0,0,10);tpGrid.Parent=tpHolder
for _,d in ipairs({{"ARRIVAL","Arrival"},{"MAIN CLUB","MainClub"},{"VIP LEVEL","VIP"},{"QUEEN SKYBOX","Queen"},{"ROOFTOP","Rooftop"},{"POOL DECK","Pool"},{"BASEMENT","Basement"}}) do local b=btn(tpHolder,d[1],UDim2.new(),UDim2.new(.48,0,0,52),Color3.fromRGB(31,25,36));stroke(b,d[2]=="Basement" and CYAN or (d[2]=="Pool" and CYAN or PINK),1,.55);b.MouseButton1Click:Connect(function()teleportRemote:FireServer(d[2]);menu.Visible=false end) end
...
local function setTab(which)
 music.Visible=which=="music";support.Visible=which=="support";teleport.Visible=which=="teleport"
 musicTab.BackgroundColor3=which=="music" and Color3.fromRGB(74,22,55) or Color3.fromRGB(26,25,31)
 supportTab.BackgroundColor3=which=="support" and Color3.fromRGB(18,65,83) or Color3.fromRGB(26,25,31)
 teleportTab.BackgroundColor3=which=="teleport" and Color3.fromRGB(74,22,55) or Color3.fromRGB(26,25,31)
end
...
stateRemote.OnClientEvent:Connect(function(kind,data)
 if kind=="playlist" then for _,c in ipairs(listHolder:GetChildren())do if c:IsA("TextButton")then c:Destroy()end end;for i,item in ipairs(data)do local b=btn(listHolder,string.format("%02d   %s",i,item.title),UDim2.new(),UDim2.new(1,-4,0,40),Color3.fromRGB(26,23,31));b.TextXAlignment=Enum.TextXAlignment.Left;b.MouseButton1Click:Connect(function()musicRemote:FireServer("play",i)end)end
 elseif kind=="music" then now.Text=(data.playing and "▶  " or "Ⅱ  ")..tostring(data.title or "")
 elseif kind=="supportProducts" then for _,c in ipairs(supportHolder:GetChildren())do if c:IsA("TextButton")then c:Destroy()end end;for i,item in ipairs(data)do local b=btn(supportHolder,tostring(item.label).." R$",UDim2.new(),UDim2.new(.48,0,0,48),Color3.fromRGB(18,65,83));b.MouseButton1Click:Connect(function()supportRemote:FireServer("prompt",i)end)end
 elseif kind=="openSupport" then menu.Visible=true;setTab("support");supportRemote:FireServer("list")
