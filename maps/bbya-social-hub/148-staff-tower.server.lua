...
local coreX=cx+18
local coreZ=cz+8
local floorSurface={34,47,60,73}
...
local C={
 ...
 woodLight=Color3.fromRGB(150,111,76),
...
-- Existing ceiling top is ~31.1. A small structural air gap keeps ownership visually separate.
local podiumCenterY=33.6
...
for _,x in ipairs({-30,-15,0,15,30}) do
 for _,z in ipairs({-18,18}) do
  part("TransferPad"..x.."_"..z,Vector3.new(1.4,2.0,1.4),CFrame.new(cx+x,32.1,cz+z),C.graphite,Enum.Material.Metal,true,podium)
 end
end
...
for _,z in ipairs({cz-PODIUM_D/2,cz+PODIUM_D/2}) do
 glass("PodiumGlassZ"..z,Vector3.new(PODIUM_W,3.6,.28),CFrame.new(cx,35.8,z),podium,.46)
 part("PodiumCapZ"..z,Vector3.new(PODIUM_W,.16,.42),CFrame.new(cx,37.62,z),C.brass,Enum.Material.Metal,false,podium)
end
for _,x in ipairs({cx-PODIUM_W/2,cx+PODIUM_W/2}) do
 glass("PodiumGlassX"..x,Vector3.new(.28,3.6,PODIUM_D),CFrame.new(x,35.8,cz),podium,.46)
 part("PodiumCapX"..x,Vector3.new(.42,.16,PODIUM_D),CFrame.new(x,37.62,cz),C.brass,Enum.Material.Metal,false,podium)
end

-- Two shallow arrival steps bridge podium top to the staff-floor surface without cutting Funkot's ceiling.
for i=1,2 do
 part("ArrivalStep"..i,Vector3.new(8,.45,2.4),CFrame.new(cx+18,33.75+i*.23,cz-13.4+i*1.6),C.stone,Enum.Material.Concrete,true,podium)
end
...
local function facadeLevel(name,floorY,nextSurfaceY,parent)
 ...
 glass("EastGlass",Vector3.new(.34,h,TOWER_D),CFrame.new(towerRight,cy,cz),m,.38)
...
for _,x in ipairs({coreRight-.35,coreLeft+.35}) do
 part("CoreWallX"..x,Vector3.new(.45,39,CORE_D),CFrame.new(x,53.5,coreZ),C.charcoal,Enum.Material.Concrete,true,core)
end
part("CoreRearWall",Vector3.new(CORE_W,39,.45),CFrame.new(coreX,53.5,coreNorth-.25),C.charcoal,Enum.Material.Concrete,true,core)
...
local function stairRun(name,lowerSurface,upperSurface,reverse,parent)
 ...
 -- Simple non-glowing handrail along the open side.
...
-- Partitions are intentionally restrained; large glass facade remains readable.
part("MeetingWall",Vector3.new(.34,8.8,13),CFrame.new(cx-8,y1+4.4,cz-8.5),C.charcoal,Enum.Material.Concrete,true,staff)
part("OfficeWall",Vector3.new(12,.34,8.8),CFrame.new(cx+6,y1+4.4,cz-2.5)*CFrame.Angles(0,0,math.rad(90)),C.charcoal,Enum.Material.Concrete,true,staff)
part("PantryWall",Vector3.new(16,.34,8.8),CFrame.new(cx+2,y1+4.4,cz+9.5)*CFrame.Angles(0,0,math.rad(90)),C.charcoal,Enum.Material.Concrete,true,staff)
...
part("RestroomBlock",Vector3.new(7,8.2,7),CFrame.new(cx+10,y1+4.1,cz+9),C.graphite,Enum.Material.Concrete,true,staff)
...
-- Central corridor along Z. Door-size gaps are left between short wall segments.
for _,x in ipairs({cx-19,cx-7,cx+5}) do
 part("CorridorSouth"..x,Vector3.new(8,.34,9.2),CFrame.new(x,y2+4.6,cz-2.5)*CFrame.Angles(0,0,math.rad(90)),C.charcoal,Enum.Material.Concrete,true,admin)
 part("CorridorNorth"..x,Vector3.new(8,.34,9.2),CFrame.new(x,y2+4.6,cz+2.5)*CFrame.Angles(0,0,math.rad(90)),C.charcoal,Enum.Material.Concrete,true,admin)
end
...
part("OfficeDivider",Vector3.new(12,.34,9.5),CFrame.new(cx+7,y3+4.75,cz-2.5)*CFrame.Angles(0,0,math.rad(90)),C.charcoal,Enum.Material.Concrete,true,owner)
part("BathDivider",Vector3.new(11,.34,9.5),CFrame.new(cx+9,y3+4.75,cz+8.5)*CFrame.Angles(0,0,math.rad(90)),C.charcoal,Enum.Material.Concrete,true,owner)
...
part("OfficeCredenza",Vector3.new(8,2.4,1.8),CFrame.new(cx+15,y3+1.2,cz-3.5),C.wood,Enum.Material.WoodPlanks,true,owner)

-- Bath / dressing east-rear.
part("BathVanity",Vector3.new(7,2.4,1.8),CFrame.new(cx+10,y3+1.2,cz+10),C.stone,Enum.Material.Marble,true,owner)
glass("ShowerGlass",Vector3.new(5.5,7,.25),CFrame.new(cx+17,y3+3.5,cz+10),owner,.45)
part("DressingBench",Vector3.new(5.6,.8,2.2),CFrame.new(cx+9,y3+.5,cz+4),C.fabric,Enum.Material.Fabric,true,owner)
...
-- Safety glass around the main roof slab.
...
-- Helipad-look marking on the west half; visual architecture only.
...
for levelIndex,y in ipairs({40.2,53.2,66.2}) do
