-- BBYA SOCIAL HUB — MEDIA CAM / PHOTO MODE v2
-- MEDIA-role-only camera authority. Server-owned managed-role attributes are the access key.
-- Camera-only: never moves Humanoid, never stops animations/emotes/dance/carry, never changes tools or character state.
...
local oldCam=pg:FindFirstChild("BBYAMediaCamUI")
if oldCam then oldCam:Destroy() end

local gui=Instance.new("ScreenGui")
gui.Name="BBYAMediaCamUI"
gui.ResetOnSpawn=false
gui.IgnoreGuiInset=true
gui.DisplayOrder=82
gui:SetAttribute("BBYAFeature","CAM")
gui:SetAttribute("RoleRequired","MEDIA")
gui:SetAttribute("LegacyPhotoLauncherRetired",true)
gui.Parent=pg
...
local function button(parent,name,text,pos,size)
 ...
 b.TextColor3=Color3.fromRGB(245,245,248);b.Font=Enum.Font.GothamBold;b.TextSize=10;b.AutoButtonColor=true;b.Parent=parent;corner(b,10);stroke(b,Color3.fromRGB(69,172,255),.35);return b
end

local toggle=button(gui,"MediaCamToggle","CAM",UDim2.new(1,-86,0,54),UDim2.fromOffset(72,36))
toggle.AnchorPoint=Vector2.new(0,0)
toggle.Visible=false

local controls=Instance.new("Frame")
controls.Name="MediaCamControls"
...
local function authorized()
 return player:GetAttribute("BBYAHasMediaRole")==true and player:GetAttribute("BBYAManagedRole")=="MEDIA"
end
...
local function syncAccess()
 local ok=authorized()
 if not ok and active then restore() end
 toggle.Visible=ok
 gui:SetAttribute("Authorized",ok)
end

local function start()
 if active or not authorized() then return end
 ...
 toggle.Text="EXIT CAM"
end

toggle.Activated:Connect(function()
 if not authorized() then syncAccess();return end
 if active then restore() else start() end
end)
...
UserInputService.InputBegan:Connect(function(input,gp)
 if not active or not authorized() then return end
...
UserInputService.TouchStarted:Connect(function(input,gp)
 if not active or not authorized() or gp then return end
...
local function hold(btn,keyName)
 btn.InputBegan:Connect(function(input)if authorized() and (input.UserInputType==Enum.UserInputType.Touch or input.UserInputType==Enum.UserInputType.MouseButton1) then touchMove[keyName]=true end end)
...
RunService.RenderStepped:Connect(function(dt)
 ...
 if not authorized() then restore();syncAccess();return end
 camera=workspace.CurrentCamera or camera
...
workspace:GetPropertyChangedSignal("CurrentCamera"):Connect(function()
 camera=workspace.CurrentCamera
 if active and camera and authorized() then camera.CameraType=Enum.CameraType.Scriptable end
end)
player:GetAttributeChangedSignal("BBYAHasMediaRole"):Connect(syncAccess)
player:GetAttributeChangedSignal("BBYAManagedRole"):Connect(syncAccess)
player.CharacterAdded:Connect(function()restore();task.defer(syncAccess)end)

syncAccess()
print("[BBYA] MEDIA CAM v2 online: MEDIA-only / camera-only / desktop+mobile / dance-carry safe")
