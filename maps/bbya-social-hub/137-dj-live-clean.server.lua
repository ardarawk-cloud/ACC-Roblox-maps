-- BBYA SOCIAL HUB — DJ LIVE CLEAN ENGINE v1
-- Clean rebuild from Arda's authoritative DJ LIVE blueprint.
-- This is a control/audio layer over existing BBYA venue SoundGroups; it does not replace venue catalog/routing authorities.
...
local AUTO_SOURCES={
 CLUB={"BBYAClubDeckA","BBYAClubDeckB"},
 VIP={"BBYAVIPPlaylist"},
 UNDERGROUND={"BBYABasementDeckA","BBYABasementDeckB"},
 FUNKOT={"BBYAFunkotRuntimeV6"},
}

-- DJ library mirrors only assets already present in active BBYA music authorities.
-- It is a read-only DJ index; venue music authorities remain the source of playback policy outside DJ LIVE.
local LIBRARY={
 -- CLUB / MAIN PROGRESSIVE
 ...
 -- VIP / EXISTING AMAPIANO AUTHORITY
 {id=105859685125263,title="Wonder Girls - Nobody (ROOKIE Amapiano Edit)",artist="VIP Amapiano",map="VIP"},
 {id=135466870455541,title="ACC_Audio_1787515147552.mp3",artist="VIP Amapiano",map="VIP"},
 {id=104570664651564,title="ACC_Audio_1787515343208.mp3",artist="VIP Amapiano",map="VIP"},

 -- UNDERGROUND / ACTIVE BKB LIBRARY
 ...
 {id=99998363156285,title="11A - 130 - RUN AWAY",artist="BBYA Underground",map="UNDERGROUND"},
 ...
 -- FUNKOT / ACTIVE VERIFIED RUNTIME
...
local function parseBpm(title)
 for n in tostring(title):gmatch("%d%d%d") do
  local bpm=tonumber(n)
  if bpm and bpm>=80 and bpm<=200 then return bpm end
 end
 return 0
end
...
local action=remotes:FindFirstChild("DJLiveAction") or Instance.new("RemoteEvent")
action.Name="DJLiveAction";action.Parent=remotes
local stateRemote=remotes:FindFirstChild("DJLiveState") or Instance.new("RemoteEvent")
stateRemote.Name="DJLiveState";stateRemote.Parent=remotes
local getState=remotes:FindFirstChild("DJLiveGetState") or Instance.new("RemoteFunction")
getState.Name="DJLiveGetState";getState.Parent=remotes
local getLibrary=remotes:FindFirstChild("DJLiveGetLibrary") or Instance.new("RemoteFunction")
getLibrary.Name="DJLiveGetLibrary";getLibrary.Parent=remotes
...
engine:SetAttribute("Version","DJ_LIVE_CLEAN_V1")
engine:SetAttribute("Blueprint","ARDA_KOTLIN_COMPOSE_EXACT_UX")
...
local function makeDeck(name)
 local s=engine:FindFirstChild(name)
...
local soundA,fxA=makeDeck("DeckA")
local soundB,fxB=makeDeck("DeckB")

local function freshDeck()
 return {trackIndex=0,title="EMPTY",artist="",assetId=0,bpm=0,playing=false,cue=0,loaded=false,fx={ECHO=false,FILTER=false,REVERB=false,FLANGER=false}}
end
local S={live=false,map="CLUB",crossfader=.5,operator=nil,notice="",decks={A=freshDeck(),B=freshDeck()}}
local suppressed={}
local lastAction={}
local brakeToken={A=0,B=0}

local function identity(p)
 if not p then return nil end
 if string.lower(p.Name)==OWNER_USERNAME then return "OWNER" end
 if p:GetAttribute("BBYAOwner")==true then return "OWNER" end
 if p:GetAttribute("BBYAAdmin")==true then return "ADMIN" end
 if game.CreatorType==Enum.CreatorType.User and p.UserId==game.CreatorId then return "CREATOR" end
 return nil
end
local function authorized(p)return identity(p)~=nil end
local function groupFor(map)
 local n=VENUE_GROUPS[map]
 local g=n and SoundService:FindFirstChild(n)
 return g and g:IsA("SoundGroup") and g or nil
end
local function deckObjects(deck)
 if deck=="A" then return soundA,fxA,S.decks.A end
 if deck=="B" then return soundB,fxB,S.decks.B end
end

local function autoSources(map)
 local out={}
 for _,name in ipairs(AUTO_SOURCES[map] or {}) do
  local s=SoundService:FindFirstChild(name)
  if s and s:IsA("Sound") then table.insert(out,s) end
 end
 return out
end
local function restoreSources()
 for s,vol in pairs(suppressed) do if s and s.Parent then s.Volume=vol end end
 suppressed={}
end
local function suppressSources(map)
 restoreSources()
 for _,s in ipairs(autoSources(map)) do suppressed[s]=s.Volume;s.Volume=0;s:SetAttribute("BBYADJLiveSuppressed",true) end
end
local function enforceSuppression()
 if not S.live then return end
 for _,s in ipairs(autoSources(S.map)) do
  if suppressed[s]==nil then suppressed[s]=s.Volume end
  s.Volume=0;s:SetAttribute("BBYADJLiveSuppressed",true)
 end
end
local function restoreAutoDefaults(map)
 for _,s in ipairs(autoSources(map)) do
  s:SetAttribute("BBYADJLiveSuppressed",false)
  if map=="CLUB" or map=="UNDERGROUND" then
   s.Volume=(s:GetAttribute("DeckRole")=="LIVE") and 1 or 0
  elseif map=="VIP" then s.Volume=.72
  elseif map=="FUNKOT" then s.Volume=.92 end
 end
end

local function routeDecks()
 local g=groupFor(S.map)
 if not g then return false end
 soundA.SoundGroup=g;soundB.SoundGroup=g
 engine:SetAttribute("Map",S.map);engine:SetAttribute("Live",S.live)
 return true
end
local function applyFx(deck)
 local _,fx,d=deckObjects(deck);if not d then return end
 for name,e in pairs(fx) do e.Enabled=d.fx[name]==true end
end
local function applyMix()
 local x=math.clamp(tonumber(S.crossfader) or .5,0,1)
 local a=math.cos(x*math.pi*.5)
 local b=math.sin(x*math.pi*.5)
 soundA.Volume=S.live and a or 0
 soundB.Volume=S.live and b or 0
 applyFx("A");applyFx("B")
end
local function syncPlayingFlags()
 S.decks.A.playing=soundA.Playing
 S.decks.B.playing=soundB.Playing
end
local function snapshot()
 syncPlayingFlags()
 return {authorized=true,version="DJ_LIVE_CLEAN_V1",live=S.live,map=S.map,crossfader=S.crossfader,operator=S.operator,notice=S.notice,decks=S.decks}
end
local function broadcast(notice)
 if notice~=nil then S.notice=tostring(notice) end
 local snap=snapshot()
 for _,p in ipairs(Players:GetPlayers()) do if authorized(p) then stateRemote:FireClient(p,snap) end end
end

local function loadTrack(deck,index)
 local sound,_,d=deckObjects(deck)
 local t=LIBRARY[tonumber(index) or 0]
 if not sound or not d or not t then return false,"Track tidak ditemukan" end
 ...
 task.spawn(function()
  ...
  if d.trackIndex==t.index then d.loaded=ok;broadcast(ok and ("Loaded Deck "..deck..": "..t.title) or ("Load failed Deck "..deck)) end
 end)
 return true
end
local function playToggle(deck)
 local sound,_,d=deckObjects(deck);if not sound or not d or d.assetId<1 then return end
 if sound.Playing then sound:Pause() else pcall(function()sound:Play()end) end
 syncPlayingFlags()
end
local function cue(deck)
 local sound,_,d=deckObjects(deck);if not sound or not d or d.assetId<1 then return end
 if sound.Playing then sound:Pause() end
 pcall(function()sound.TimePosition=math.max(0,d.cue or 0)end)
 syncPlayingFlags()
end
local function syncDeck(deck)
 local target,_,td=deckObjects(deck);local other=deck=="A" and "B" or "A";local source,_,sd=deckObjects(other)
 ...
 if source.Playing then
  ...
  pcall(function()target.TimePosition=math.max(0,math.floor(target.TimePosition/2)*2+phase)end)
 end
end
local function toggleFx(deck,name)
 name=string.upper(tostring(name or ""));local _,_,d=deckObjects(deck)
 if not d or d.fx[name]==nil then return end
 d.fx[name]=not d.fx[name];applyFx(deck)
end
local function brake(deck)
 local sound,_,d=deckObjects(deck);if not sound or not d or d.assetId<1 then return end
 brakeToken[deck]+=1;local token=brakeToken[deck];local start=math.max(.8,sound.PlaybackSpeed)
 task.spawn(function()
  for i=1,12 do
   if brakeToken[deck]~=token then return end
   sound.PlaybackSpeed=math.max(.08,start*(1-i/13));task.wait(.045)
  end
  if brakeToken[deck]==token then sound:Pause();sound.PlaybackSpeed=1;syncPlayingFlags();broadcast("BRAKE • DECK "..deck) end
 end)
end
...
local function findExistingSample(name)
 ...
 for _,root in ipairs({SoundService,ReplicatedStorage}) do
  for _,d in ipairs(root:GetDescendants()) do
   if d:IsA("Sound") and d~=soundA and d~=soundB then
    if normalized(d.Name)==wanted or normalized(d:GetAttribute("BBYASFXName"))==wanted then return d end
   end
  end
 end
end
local function triggerSample(deck,name)
 ...
 local source=findExistingSample(name)
 if source and source.SoundId~="" then
  local clone=Instance.new("Sound");clone.Name="DJLiveSample_"..name;clone.SoundId=source.SoundId;clone.Volume=source.Volume>0 and source.Volume or 1;clone.PlaybackSpeed=source.PlaybackSpeed;clone.SoundGroup=groupFor(S.map);clone.Parent=engine
  clone.Ended:Connect(function()if clone.Parent then clone:Destroy() end end)
  Debris:AddItem(clone,20);pcall(function()clone:Play()end);broadcast(name.." • DECK "..deck);return
 end
 if name=="BRAKE" then brake(deck);return end
 broadcast(name.." sample belum tersedia di BBYA SFX authority")
end

local function setMap(map)
 map=string.upper(tostring(map or ""));if not VENUE_GROUPS[map] then return false end
 local g=groupFor(map);if not g then broadcast("Audio master "..map.." belum tersedia");return false end
 if S.live then local old=S.map;restoreSources();restoreAutoDefaults(old);S.map=map;suppressSources(map) else S.map=map end
 routeDecks();applyMix();return true
end
local function liveStart(player)
 if S.live then return end
 if not routeDecks() then broadcast("Venue master belum siap");return end
 S.live=true;S.operator=identity(player);suppressSources(S.map);applyMix();broadcast("DJ LIVE START • "..S.map)
end
local function liveStop()
 if not S.live then return end
 local old=S.map;S.live=false;S.operator=nil;restoreSources();restoreAutoDefaults(old);soundA:Pause();soundB:Pause();applyMix();broadcast("DJ LIVE STOP")
end
...
local function handle(player,kind,payload)
 ...
 kind=string.lower(tostring(kind or ""));payload=type(payload)=="table" and payload or {}
 ...
 elseif kind=="live_stop" then liveStop()
 else return end
 broadcast()
end
...
local function applyAuth(p)
 local id=identity(p);p:SetAttribute("BBYADJLiveAuthorized",id~=nil);p:SetAttribute("BBYADJLiveIdentity",id or "")
end
...
Players.PlayerRemoving:Connect(function(p)
 lastAction[p]=nil
 task.defer(function()
  if S.live then for _,other in ipairs(Players:GetPlayers()) do if authorized(other) then return end end;liveStop() end
 end)
end)

soundA.Ended:Connect(function()syncPlayingFlags();broadcast()end)
soundB.Ended:Connect(function()syncPlayingFlags();broadcast()end)
task.spawn(function()
 while task.wait(.10) do if S.live then enforceSuppression();applyMix() end end
end)
task.spawn(function()
 while task.wait(.35) do if S.live or soundA.Playing or soundB.Playing then broadcast() end end
end)
game:BindToClose(function()restoreSources();restoreAutoDefaults(S.map)end)
routeDecks();applyMix()
print("[BBYA] DJ LIVE CLEAN ENGINE v1 online: Deck A/B + crossfader + venue routing + FX + SFX adapter")