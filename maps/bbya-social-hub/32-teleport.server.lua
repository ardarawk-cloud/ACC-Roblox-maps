-- BBYA SOCIAL HUB — TRAVEL / ONE-TIME ACCESS v8
...
local destinations={Arrival=CFrame.new(0,4,-58),Photo=CFrame.new(-39,3,-25),LookLab=CFrame.new(-38,3,-4),MainClub=CFrame.new(3,3,11),Toilet=CFrame.new(43,3,-13),VIP=CFrame.new(46,27,2),Rooftop=CFrame.new(43,47,-28),Pool=CFrame.new(0,47,-12),Basement=CFrame.new(0,-12,0),Skatepark=CFrame.new(0,3,112),Funkot=CFrame.new(0,3,178),Mall=CFrame.new(0,4,302),NightMarket=CFrame.new(0,4,482)}
...
local function doTeleport(player,key)
 local cf=destinations[key];if not cf then return false end
 local char=player and player.Character;local hrp=char and char:FindFirstChild("HumanoidRootPart");if not hrp then return false end
 hrp.CFrame=cf;hrp.AssemblyLinearVelocity=Vector3.zero;hrp.AssemblyAngularVelocity=Vector3.zero;return true
end
local function owns(player,key)
 if hasRoleBypass(player,key) then return true end
 local passId=tonumber(PASSES[key]) or 0;if passId<=0 then return false end
 ownershipCache[player.UserId]=ownershipCache[player.UserId] or {};local cached=ownershipCache[player.UserId][key]
...
internal.Event:Connect(function(player,key)if doTeleport(player,key) then toast(player,tostring(key).." access ready.") end end)
tp.OnServerEvent:Connect(function(player,key)
 key=tostring(key or "");if not destinations[key] then return end
 local price=PRICES[key];if not price then doTeleport(player,key);return end
 if owns(player,key) then doTeleport(player,key);return end
 local passId=tonumber(PASSES[key]) or 0
 if passId<=0 then toast(player,"One-time access sedang sinkron. Coba lagi sebentar.");return end
 player:SetAttribute("BBYAPendingTravelPass",key);MarketplaceService:PromptGamePassPurchase(player,passId)
end)
MarketplaceService.PromptGamePassPurchaseFinished:Connect(function(player,passId,purchased)
 if not player or not purchased then return end
 local key=keyByPass[tonumber(passId) or 0];if not key then return end
 ownershipCache[player.UserId]=ownershipCache[player.UserId] or {};ownershipCache[player.UserId][key]=true;player:SetAttribute("BBYAPendingTravelPass",nil)
...
print("[BBYA] Travel v8 online: paid access preserved + VIP/CREW/ADMIN managed role bypass support")
