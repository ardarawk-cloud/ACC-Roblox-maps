-- BBYA SOCIAL HUB — MALL CINEMA + REAR LIFT v13 / L4 ACCESS v15
-- Screenshot-driven Mall-only refinement aligned to the true 15-stud-clear Mall stack.
...
-- Wait for the active 15-clear Mall overlay, but do not resurrect legacy V11 datums.
local deadline=os.clock()+120
while mall:GetAttribute("MallCentralLift")~="V12_15_CLEAR" and os.clock()<deadline do task.wait(.25) end
...
elevator:SetAttribute("Pass","REAR_WALL_LIFT_V13_15_CLEAR")
...
part("LiftRearPanel",Vector3.new(16.5,65,.6),CFrame.new(0,32.5,rearZ),C.glass,Enum.Material.Glass,true,elevator,.38)
for _,x in ipairs({-8.2,8.2}) do
 part("LiftPier"..x,Vector3.new(1.2,65,8.5),CFrame.new(x,32.5,438.0),C.graphite,Enum.Material.Metal,true,elevator,0)
end
...
for i,y in ipairs(levels) do
 ...
 part("LiftDoorL"..i,Vector3.new(6.2,10.5,.35),CFrame.new(-3.15,y+6.1,doorZ),C.dark,Enum.Material.Metal,true,elevator,0)
 part("LiftDoorR"..i,Vector3.new(6.2,10.5,.35),CFrame.new(3.15,y+6.1,doorZ),C.dark,Enum.Material.Metal,true,elevator,0)
 local header=part("LiftHeader"..i,Vector3.new(15,2.25,.28),CFrame.new(0,y+12.15,doorZ-.1),C.graphite,Enum.Material.Metal,false,elevator,0)
...
mall:SetAttribute("MallCentralLift","V13_REAR_WALL_15_CLEAR")
...
if cinema then
 ...
 local hero=part("CinemaHeroV12",Vector3.new(42,4,.35),CFrame.new(0,y+13.2,392.2),C.dark,Enum.Material.Metal,false,cxModel,0)
 ...
 local snackSign=part("CinemaSnackSign",Vector3.new(11,2,.25),CFrame.new(0,y+7.1,397.65),C.dark,Enum.Material.Metal,false,cxModel,0)
 ...
 local function buildScreenRoom(label,cx)
  ...
  part("LeftWall",Vector3.new(1,14,depth),CFrame.new(cx-half,y+8,centerZ),C.dark,Enum.Material.Slate,true,room,0)
  part("RightWall",Vector3.new(1,14,depth),CFrame.new(cx+half,y+8,centerZ),C.dark,Enum.Material.Slate,true,room,0)
  part("RearWall",Vector3.new(width,14,1),CFrame.new(cx,y+8,rearWallZ),C.dark,Enum.Material.Slate,true,room,0)
  part("Ceiling",Vector3.new(width,.45,depth),CFrame.new(cx,y+14.5,centerZ),Color3.fromRGB(33,34,38),Enum.Material.Metal,true,room,0)

  local entryHead=part("EntryHeader",Vector3.new(15,2.4,.32),CFrame.new(cx,y+12.2,frontZ-.55),C.graphite,Enum.Material.Metal,false,room,0)
  textOn(entryHead,"SCREEN "..label,C.gold)

  local frame=part("ScreenFrame",Vector3.new(54,8.8,.55),CFrame.new(cx,y+8.2,rearWallZ-.65),Color3.fromRGB(9,10,12),Enum.Material.Metal,false,room,0)
  local screen=part("MovieScreen",Vector3.new(50,6.8,.18),CFrame.new(cx,y+8.2,rearWallZ-1.0),Color3.fromRGB(220,220,216),Enum.Material.SmoothPlastic,false,room,0)
 ...
 cinema:SetAttribute("Experience","REAL_SCREENING_ROOMS_V13_15_CLEAR")
 cinema:SetAttribute("PortalOnlyTheatresRemoved",true)
 cinema:SetAttribute("L4Circulation","V15_ATRIUM_ACCESS")
 mall:SetAttribute("MallCinemaExperience","V13_V15_ACCESS")
end
...
if floor4 then
 ...
 floor4:SetAttribute("AtriumSafety","V15_CINEMA_ACCESS")
 ...
 mall:SetAttribute("MallL4AtriumSafety","V15_CINEMA_ACCESS")
end
...
pass:SetAttribute("FloorClearHeightStuds",15)
mall:SetAttribute("MallScreenshotQC","V15_15_CLEAR")
print("[BBYA] Mall v15 online: rear lift + Cinema aligned to true 15-stud-clear Mall datums")
