-- BBYA SOCIAL HUB — COMMUNITY + OWNER + TOP 3 DONATOR v1
...
model:SetAttribute("Pass","COMMUNITY_OWNER_TOP3_V1")
...
local C={
 ...
 BRONZE=Color3.fromRGB(203,133,85),GREEN=Color3.fromRGB(72,211,133),BLACK=Color3.fromRGB(3,3,5),
}
...
local function image(parent,pos,size)
 local i=Instance.new("ImageLabel");i.BackgroundColor3=C.PANEL2;i.BackgroundTransparency=0;i.BorderSizePixel=0;i.Position=pos;i.Size=size;i.ScaleType=Enum.ScaleType.Crop;i.Parent=parent;corner(i,999);stroke(i,C.CYAN,2,.15);return i
end
...
local function makeBoard(name,x,accentA,accentB)
 ...
 face.Reflectance=.03
 -- All neon is inset inside the display footprint to prevent facade clipping.
...
-- LEFT: COMMUNITY + OWNER -----------------------------------------------------
local leftHolder,leftFace,left=makeBoard("CommunityOwnerWall",-34.5,C.PINK,C.CYAN)
...
for i=1,4 do
 ...
 local dot=frame(row,UDim2.fromScale(.025,.29),UDim2.fromScale(.035,.42),i==1 and C.PINK or C.CYAN,0,99)
...
-- RIGHT: TOP 3 DONATOR --------------------------------------------------------
local rightHolder,rightFace,right=makeBoard("Top3DonatorWall",34.5,C.GOLD,C.PINK)
...
-- DONOR LEDGER ---------------------------------------------------------------
-- Support total is persisted independently from rank so static verified pass contribution can be added safely.
local loadedSupport={}
local saveToken={}
local wiring={}
...
local function verifiedSultan(p)
 -- This value must be set by a real server-side Sultan authority. No ID or price is invented here.
 return numeric(p and p:GetAttribute("BBYASultanContributionRobux"))
end
local function combinedFor(p)
 return numeric(p and p:GetAttribute("BBYASupportRobuxTotal"))+verifiedSultan(p)
end
 ...
 task.spawn(function()
  ...
  for i=1,3 do
   ...
   if q then
    ...
    ref.amount.Text=string.format("%,d R$",q.total):gsub(",",".")
...
Players.PlayerRemoving:Connect(function(p)
 persistPlayer(p);seen[p.UserId]=nil;loadedSupport[p.UserId]=nil;saveToken[p.UserId]=nil;wiring[p.UserId]=nil
end)
...
print("[BBYA] Community + Owner + Top 3 Donator v1 online: readable neon / real donor ranking / >1000 eligibility")