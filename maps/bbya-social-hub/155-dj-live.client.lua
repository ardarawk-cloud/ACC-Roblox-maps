...
local function modal(titleText)
 ...
 local box=frame(shade,"Dialog",UDim2.fromScale(.5,.50),UDim2.new(.76,0,.74,0),C.panel,.02,13,101);box.AnchorPoint=Vector2.new(.5,.5)
...
local function openPlaylist(deck)
 ...
 local function render()
  ...
  for _,t in ipairs(library)do if tostring(t.map)==tostring(state.map)then local blob=string.lower(tostring(t.title or"").." "..tostring(t.artist or""));if q==""or string.find(blob,q,1,true)then count+=1;local suffix=(tonumber(t.bpm)or 0)>0 and("   •   "..tostring(t.bpm).." BPM")or"";local r=button(list,"Track"..tostring(t.index),tostring(t.title)..suffix,nil,UDim2.new(1,-4,0,40),C.card,103);r.LayoutOrder=count;r.TextXAlignment=Enum.TextXAlignment.Left;r.TextSize=8;local pad=Instance.new("UIPadding");pad.PaddingLeft=UDim.new(0,10);pad.PaddingRight=UDim.new(0,8);pad.Parent=r;r.Activated:Connect(function()action:FireServer("load",{deck=deck,index=t.index});clearDialog()end)end end end
  if count==0 then local empty=label(list,"Empty","NO TRACKS FOR THIS VENUE",nil,UDim2.new(1,-4,0,40),Enum.Font.GothamBold,9,C.muted,Enum.TextXAlignment.Center,103);empty.LayoutOrder=1 end
 end
...
local function createTimeline(parent,deck,accent)
 local w=frame(parent,"TrackTimeline",UDim2.fromOffset(10,58),UDim2.new(1,-20,0,52),C.black,.02,7);w.ClipsDescendants=true;w:SetAttribute("TimelineType","TIMEPOSITION_TIMELENGTH")
 label(w,"TimelineLabel","TRACK TIMELINE",UDim2.fromOffset(7,1),UDim2.new(1,-14,0,11),Enum.Font.GothamBold,6,C.muted)
 local strip=Instance.new("Frame");strip.Name="Segments";strip.BackgroundTransparency=1;strip.Position=UDim2.fromOffset(6,14);strip.Size=UDim2.new(1,-12,0,20);strip.ClipsDescendants=true;strip.Parent=w
 local segs={};local count=48
 for i=1,count do local b=Instance.new("Frame");b.AnchorPoint=Vector2.new(.5,.5);b.Position=UDim2.new((i-.5)/count,0,.5,0);b.Size=UDim2.new(1/count,-1,0,14);b.BackgroundColor3=accent;b.BackgroundTransparency=.78;b.BorderSizePixel=0;b.Parent=strip;segs[i]=b end
 local playhead=Instance.new("Frame");playhead.Name="Playhead";playhead.AnchorPoint=Vector2.new(.5,.5);playhead.Position=UDim2.new(0,0,.5,0);playhead.Size=UDim2.fromOffset(2,20);playhead.BackgroundColor3=C.white;playhead.BorderSizePixel=0;playhead.ZIndex=3;playhead.Parent=strip
 local elapsed=label(w,"Elapsed","00:00",UDim2.fromOffset(7,35),UDim2.fromOffset(52,13),Enum.Font.GothamBold,6,C.muted)
...
local function makeDeck(deck,order,accent)
 ...
 local title=label(f,"TrackTitle","EMPTY",UDim2.fromOffset(102,6),UDim2.new(1,-176,0,22),Enum.Font.GothamBlack,10,C.white)
...
local mixer=frame(content,"Mixer",nil,UDim2.new(0,116,1,0),C.panel,.08,11);mixer.LayoutOrder=2;mixer.ClipsDescendants=true
...
local meters=Instance.new("Frame");meters.Name="Meters";meters.Position=UDim2.fromOffset(18,38);meters.Size=UDim2.new(1,-36,1,-118);meters.BackgroundTransparency=1;meters.Parent=mixer
for i=1,14 do for side,deck in ipairs({"A","B"})do local b=Instance.new("Frame");b.AnchorPoint=Vector2.new(0,1);b.Position=UDim2.new(side==1 and 0 or .56,0,1,-(i-1)*7);b.Size=UDim2.new(.40,0,0,5);b.BackgroundColor3=side==1 and C.pink or C.cyan;b.BackgroundTransparency=.78;b.BorderSizePixel=0;b.Parent=meters;corner(b,2);meterBars[deck][i]=b end end
local rail=Instance.new("Frame");rail.Name="CrossfaderRail";rail.Position=UDim2.new(0,14,1,-54);rail.Size=UDim2.new(1,-28,0,5);rail.BackgroundColor3=C.line;rail.BorderSizePixel=0;rail.Parent=mixer;corner(rail,3)
...
label(mixer,"CrossLabel","CROSSFADER",UDim2.new(0,6,1,-32),UDim2.new(1,-12,0,14),Enum.Font.GothamBold,7,C.muted,Enum.TextXAlignment.Center)
