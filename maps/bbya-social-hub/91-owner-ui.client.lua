-- BBYA SOCIAL HUB — ROLE PANEL v9 FUNCTION-ONLY
...
local panel=Instance.new("Frame");panel.Name="RolePanel";panel.AnchorPoint=Vector2.new(1,0);panel.Position=UDim2.new(1,-96,0,8);panel.Size=UDim2.fromOffset(540,438);panel.BackgroundColor3=C.bg;panel.BorderSizePixel=0;panel.ZIndex=151;panel.Parent=shade;corner(panel,12);stroke(panel,C.stroke,.08)
panel:SetAttribute("BBYAPositionAuthority","ROLE_PANEL_V9_SELF")
...
text(panel,"SubTitle","Persistent roles • DJ / LEAD / MEDIA own their feature access",UDim2.fromOffset(430,18),UDim2.fromOffset(18,39),Enum.Font.Gotham,11,C.muted)
...
local list=Instance.new("ScrollingFrame");list.Name="PlayerList";list.Position=UDim2.fromOffset(16,100);list.Size=UDim2.fromOffset(235,318);list.BackgroundColor3=C.panel;list.BorderSizePixel=0;list.ScrollBarThickness=3;list.ScrollBarImageColor3=C.gold;list.CanvasSize=UDim2.new();list.AutomaticCanvasSize=Enum.AutomaticSize.Y;list.ZIndex=152;list.Parent=panel;corner(list,9);stroke(list,C.stroke,.35)
...
local detail=Instance.new("Frame");detail.Name="RoleDetail";detail.Position=UDim2.fromOffset(263,78);detail.Size=UDim2.fromOffset(261,340);detail.BackgroundColor3=C.panel;detail.BorderSizePixel=0;detail.ZIndex=152;detail.Parent=panel;corner(detail,9);stroke(detail,C.stroke,.35)
...
local rule=text(detail,"Rule","DJ: DJ LIVE only • LEAD: Dance Sync • MEDIA: CAM\nVIP / CREW / ADMIN keep existing access behavior",UDim2.new(1,-24,0,48),UDim2.fromOffset(12,85),Enum.Font.Gotham,10,C.muted);rule.TextWrapped=true;rule.TextYAlignment=Enum.TextYAlignment.Top

local vip=button(detail,"VIP","VIP",UDim2.fromOffset(112,34),UDim2.fromOffset(12,138),C.gold)
local crew=button(detail,"CREW","CREW",UDim2.fromOffset(112,34),UDim2.fromOffset(137,138),C.green)
local admin=button(detail,"ADMIN","ADMIN",UDim2.fromOffset(112,34),UDim2.fromOffset(12,179),C.cyan)
local dj=button(detail,"DJ","DJ",UDim2.fromOffset(112,34),UDim2.fromOffset(137,179),C.purple)
local lead=button(detail,"LEAD","LEAD",UDim2.fromOffset(112,34),UDim2.fromOffset(12,220),C.lead)
local media=button(detail,"MEDIA","MEDIA",UDim2.fromOffset(112,34),UDim2.fromOffset(137,220),C.media)
local remove=button(detail,"REMOVE","REMOVE ROLE",UDim2.fromOffset(237,34),UDim2.fromOffset(12,261),C.red)
local status=text(detail,"Status","Select an online player.",UDim2.new(1,-24,0,34),UDim2.fromOffset(12,301),Enum.Font.Gotham,10,C.muted);status.TextWrapped=true

local selectedUserId=nil
local selectedLocked=false
local currentSnapshot=snapshot
local function roleColor(role)
 if role=="OWNER" then return C.pink elseif role=="ADMIN" then return C.cyan elseif role=="CREW" then return C.green elseif role=="VIP" then return C.gold elseif role=="DJ" then return C.purple elseif role=="LEAD" then return C.lead elseif role=="MEDIA" then return C.media end
 return C.muted
end
local roleButtons={vip,crew,admin,dj,lead,media,remove}
...
local function renderSnapshot(snap)
 ...
 for i,entry in ipairs(snap.players or {}) do
  ...
  local r=text(row,"Role",entry.role,UDim2.fromOffset(72,18),UDim2.new(1,-79,0,15),Enum.Font.GothamBold,9,roleColor(entry.role),Enum.TextXAlignment.Right)
...
vip.Activated:Connect(function()assign("VIP")end);crew.Activated:Connect(function()assign("CREW")end);admin.Activated:Connect(function()assign("ADMIN")end)
dj.Activated:Connect(function()assign("DJ")end);lead.Activated:Connect(function()assign("LEAD")end);media.Activated:Connect(function()assign("MEDIA")end);remove.Activated:Connect(function()assign("NONE")end)
...
local function resizeRolePanel()
 ...
 local scale=math.clamp(math.min(vp.X/610,vp.Y/520),.58,1);if not UserInputService.TouchEnabled then scale=math.clamp(scale,.80,1) end
...
print("[BBYA] Role Panel v9 online: VIP/CREW/ADMIN + DJ/LEAD/MEDIA persistent role authority")
