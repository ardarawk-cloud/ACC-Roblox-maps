-- BBYA SOCIAL HUB — MESSAGE + MONETIZATION AUTHORITY v6 CLEAN FLOW
-- Clean rebuild after repeated QC: fast feedback, short anti-spam cooldown, one receipt authority.
-- Exact verified Developer Product IDs remain locked. Support receipt handling remains in this file.
...
local messageProductByAmount,messageAmountByProduct,supportProductByAmount,supportAmountByProduct={},{},{},{}
...
root:SetAttribute("BBYAMonetizationAuthority","MESSAGE_V6_CLEAN_FLOW_VERIFIED_IDS")
root:SetAttribute("BBYAMessageProductCount",9);root:SetAttribute("BBYASupportProductCount",8);root:SetAttribute("BBYAVerifiedProductIdsAuthoritative",true)
...
local model=Instance.new("Model");model.Name="DJWallMessageSystem";model:SetAttribute("Pass","MESSAGE_MONETIZATION_V6_CLEAN_FLOW");model:SetAttribute("PublicName","MESSAGE");model:SetAttribute("MessageTierCount",#MESSAGE_AMOUNTS);model.Parent=root
local function part(name,size,cf,color,material,tr)local p=Instance.new("Part");p.Name=name;p.Size=size;p.CFrame=cf;p.Color=color or C.black;p.Material=material or Enum.Material.Metal;p.Transparency=tr or 0;p.Anchored=true;p.CanCollide=false;p.CanTouch=false;p.CanQuery=true;p.CastShadow=false;p.Parent=model;return p end
...
local function lab(parent,txt,pos,size,font,ts,color,align)local l=Instance.new("TextLabel");l.BackgroundTransparency=1;l.Text=txt;l.Position=pos;l.Size=size;l.Font=font or Enum.Font.Gotham;l.TextSize=ts or 18;l.TextColor3=color or C.white;l.TextWrapped=true;l.TextXAlignment=align or Enum.TextXAlignment.Left;l.TextYAlignment=Enum.TextYAlignment.Center;l.Parent=parent;return l end
...
local DISPLAY_SECONDS=9
local MAX_CHARS=80
local SUBMIT_COOLDOWN=8
local MAX_QUEUE=20
local queue,pending,lastSubmit={},{},{}
local displaying=false
local CATEGORY={BIRTHDAY="BIRTHDAY CELEBRATION",LOVE="LOVE MESSAGE",SHOUTOUT="SHOUTOUT",CUSTOM="LIVE MESSAGE"}
local function isAdmin(p)return p:GetAttribute("BBYAAdmin")==true or p:GetAttribute("BBYAOwner")==true or(game.CreatorType==Enum.CreatorType.User and p.UserId==game.CreatorId)end
...
task.spawn(function()while task.wait(.15)do if not displaying and #queue>0 then showMessage(table.remove(queue,1))end end end)
local function tierAvailability()local a={};for _,amount in ipairs(MESSAGE_AMOUNTS)do a[amount]=messageProductByAmount[amount]~=nil end;return a end
local function configFor(p)return{publicName="MESSAGE",tiers=MESSAGE_AMOUNTS,available=tierAvailability(),maxChars=MAX_CHARS,displaySeconds=DISPLAY_SECONDS,queue=#queue,admin=isAdmin(p),verifiedProducts=true,cooldownSeconds=SUBMIT_COOLDOWN}end
...
wallRemote.OnServerEvent:Connect(function(p,actionName,data)
 ...
 local now=os.clock();local remain=SUBMIT_COOLDOWN-(now-(lastSubmit[p.UserId]or 0))
 if remain>0 and not isAdmin(p)then wallRemote:FireClient(p,"toast","Tunggu "..tostring(math.ceil(remain)).." detik.");return end
 if pending[p.UserId]then wallRemote:FireClient(p,"toast","Selesaikan purchase MESSAGE sebelumnya.");return end
 if #queue>=MAX_QUEUE then wallRemote:FireClient(p,"toast","Antrean MESSAGE sedang penuh.");return end
 wallRemote:FireClient(p,"processing",{message="Memeriksa pesan..."})
 ...
 if isAdmin(p)then if queueMessage(entry)then wallRemote:FireClient(p,"queued",{position=#queue,text=filtered,amount=0,adminPreview=true})else wallRemote:FireClient(p,"toast","Antrean penuh.")end;return end
 ...
 local okPrompt=pcall(function()MarketplaceService:PromptProductPurchase(p,productId)end);if not okPrompt then pending[p.UserId]=nil;wallRemote:FireClient(p,"toast","Purchase prompt gagal dibuka.")end
end)
...
MarketplaceService.ProcessReceipt=function(receipt)
 ...
 if amount then local p=Players:GetPlayerByUserId(receipt.PlayerId);if p then local total=(tonumber(p:GetAttribute("BBYASupportRobuxTotal"))or 0)+amount;p:SetAttribute("BBYASupportRobuxTotal",total);monetizationRemote:FireClient(p,"receipt",{amount=amount,total=total});stateRemote:FireAllClients("supportReceived",{displayName=p.DisplayName,userId=p.UserId,amount=amount,total=total})end;return Enum.ProductPurchaseDecision.PurchaseGranted end
 return Enum.ProductPurchaseDecision.NotProcessedYet
end
Players.PlayerRemoving:Connect(function(p)pending[p.UserId]=nil;lastSubmit[p.UserId]=nil end)
print("[BBYA] MESSAGE + Monetization v6 online: clean fast flow / 8s cooldown / exact Product IDs / single receipt authority")