-- BBYA SOCIAL HUB — PLAYER IDENTITY UI v1
...
local gui=Instance.new("ScreenGui");gui.Name="BBYAIdentityUI";gui.ResetOnSpawn=false;gui.IgnoreGuiInset=true;gui.DisplayOrder=175;gui.Parent=pg
...
local shade=Instance.new("Frame");shade.Size=UDim2.fromScale(1,1);shade.BackgroundColor3=Color3.new(0,0,0);shade.BackgroundTransparency=.78;shade.Visible=false;shade.BorderSizePixel=0;shade.Parent=gui
local panel=Instance.new("Frame");panel.AnchorPoint=Vector2.new(1,0);panel.Position=UDim2.new(1,-18,0,58);panel.Size=UDim2.fromOffset(330,330);panel.BackgroundColor3=C.bg;panel.BorderSizePixel=0;panel.Parent=shade;corner(panel,14);stroke(panel,C.pink,.28)
text(panel,"CUSTOM TITLE",UDim2.fromOffset(16,12),UDim2.new(1,-70,0,24),Enum.Font.GothamBlack,17,C.white)
text(panel,"Cosmetic only • max 16 characters",UDim2.fromOffset(16,36),UDim2.new(1,-70,0,18),Enum.Font.Gotham,9,C.muted)
local close=btn(panel,"×",UDim2.new(1,-46,0,12),UDim2.fromOffset(32,30),C.card);close.TextSize=18
local titleBox=Instance.new("TextBox");titleBox.Position=UDim2.fromOffset(16,70);titleBox.Size=UDim2.new(1,-32,0,40);titleBox.BackgroundColor3=C.card;titleBox.BorderSizePixel=0;titleBox.PlaceholderText="BOCIL ROBLOX";titleBox.PlaceholderColor3=C.muted;titleBox.TextColor3=C.white;titleBox.ClearTextOnFocus=false;titleBox.Font=Enum.Font.GothamSemibold;titleBox.TextSize=12;titleBox.Text="";titleBox.Parent=panel;corner(titleBox,9);stroke(titleBox,C.pink,.58);local p1=Instance.new("UIPadding");p1.PaddingLeft=UDim.new(0,12);p1.PaddingRight=UDim.new(0,12);p1.Parent=titleBox
local colorBox=Instance.new("TextBox");colorBox.Position=UDim2.fromOffset(16,120);colorBox.Size=UDim2.fromOffset(112,36);colorBox.BackgroundColor3=C.card;colorBox.BorderSizePixel=0;colorBox.PlaceholderText="#FFFFFF";colorBox.PlaceholderColor3=C.muted;colorBox.TextColor3=C.white;colorBox.ClearTextOnFocus=false;colorBox.Font=Enum.Font.GothamBold;colorBox.TextSize=11;colorBox.Text="#F3F3F3";colorBox.Parent=panel;corner(colorBox,9);stroke(colorBox,C.cyan,.58);local p2=Instance.new("UIPadding");p2.PaddingLeft=UDim.new(0,10);p2.Parent=colorBox
local preview=Instance.new("Frame");preview.Position=UDim2.fromOffset(140,120);preview.Size=UDim2.new(1,-156,0,68);preview.BackgroundColor3=C.panel;preview.BorderSizePixel=0;preview.Parent=panel;corner(preview,9);stroke(preview,C.muted,.6)
text(preview,"PREVIEW",UDim2.fromOffset(10,3),UDim2.new(1,-20,0,18),Enum.Font.GothamBold,8,C.muted)
local previewText=text(preview,"YOUR TITLE",UDim2.fromOffset(10,22),UDim2.new(1,-20,0,34),Enum.Font.GothamBlack,14,C.white,Enum.TextXAlignment.Center)
local save=btn(panel,"SAVE",UDim2.fromOffset(16,202),UDim2.fromOffset(92,38),C.pink)
local equip=btn(panel,"EQUIP",UDim2.fromOffset(119,202),UDim2.fromOffset(92,38),Color3.fromRGB(45,86,72))
local remove=btn(panel,"REMOVE",UDim2.fromOffset(222,202),UDim2.fromOffset(92,38),Color3.fromRGB(83,38,47))
local status=text(panel,"TITLE not loaded yet.",UDim2.fromOffset(16,250),UDim2.new(1,-32,0,52),Enum.Font.Gotham,10,C.muted);status.TextWrapped=true;status.TextYAlignment=Enum.TextYAlignment.Top
...
local function closePanel()shade.Visible=false end
...
local function injectMenuButton()
 ...
 b.Parent=list;b:SetAttribute("BBYAInjectedAuthority","IDENTITY_UI_V1");b.Activated:Connect(function()if drawer and drawer:IsA("GuiObject") then drawer.Visible=false end;shade.Visible=true;loadSnapshot()end);menuButton=b;return true
end
...
local levelPopup=Instance.new("Frame");levelPopup.AnchorPoint=Vector2.new(.5,0);levelPopup.Position=UDim2.new(.5,0,.14,-120);levelPopup.Size=UDim2.fromOffset(370,94);levelPopup.BackgroundColor3=C.bg;levelPopup.BorderSizePixel=0;levelPopup.Visible=false;levelPopup.Parent=gui;corner(levelPopup,16);stroke(levelPopup,C.gold,.20)
local levelAvatar=Instance.new("ImageLabel");levelAvatar.Position=UDim2.fromOffset(14,15);levelAvatar.Size=UDim2.fromOffset(64,64);levelAvatar.BackgroundColor3=C.card;levelAvatar.BorderSizePixel=0;levelAvatar.ScaleType=Enum.ScaleType.Crop;levelAvatar.Parent=levelPopup;corner(levelAvatar,32)
text(levelPopup,"LEVEL UP!",UDim2.fromOffset(94,14),UDim2.new(1,-108,0,27),Enum.Font.GothamBlack,18,C.gold)
local levelText=text(levelPopup,"You reached Level 1",UDim2.fromOffset(94,43),UDim2.new(1,-108,0,23),Enum.Font.GothamBold,13,C.white)
local rankText=text(levelPopup,"NEWBIE",UDim2.fromOffset(94,67),UDim2.new(1,-108,0,14),Enum.Font.GothamBold,9,C.cyan)
...
print("[BBYA] Identity UI v1 online: custom TITLE + persistent server actions + actual Level Up popup")
