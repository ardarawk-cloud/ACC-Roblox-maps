-- BBYA SOCIAL HUB — PLAYER IDENTITY UI v1.2 GENERAL PANEL LOCK
-- Custom TITLE editor + Level Up popup. Server progression authority remains 92-player-progression.server.lua.
-- Runtime QC lock: TITLE editor follows the same 340px General Panel footprint; X is the only close control.
...
gui:SetAttribute("TitleClickLayerAuthority","IDENTITY_UI_V1_2_GENERAL_PANEL")
...
local function kernelMenuVisible(visible)
 ...
 if m and m:IsA("GuiObject") then m.Visible=visible;if m:IsA("TextButton") then m.Text="MENU" end end
end
...
local panel=Instance.new("Frame");panel.AnchorPoint=Vector2.new(1,.5);panel.Position=UDim2.new(1,-12,.5,20);panel.Size=UDim2.fromOffset(340,340);panel.BackgroundColor3=C.bg;panel.BackgroundTransparency=.08;panel.BorderSizePixel=0;panel.Active=true;panel.ClipsDescendants=true;panel.ZIndex=201;panel.Parent=shade;corner(panel,14);stroke(panel,C.pink,.28)
...
local colorBox=Instance.new("TextBox");colorBox.Position=UDim2.fromOffset(16,120);colorBox.Size=UDim2.fromOffset(116,36);colorBox.BackgroundColor3=C.card;colorBox.BorderSizePixel=0;colorBox.PlaceholderText="#FFFFFF";colorBox.PlaceholderColor3=C.muted;colorBox.TextColor3=C.white;colorBox.ClearTextOnFocus=false;colorBox.Font=Enum.Font.GothamBold;colorBox.TextSize=11;colorBox.Text="#F3F3F3";colorBox.Active=true;colorBox.ZIndex=204;colorBox.Parent=panel;corner(colorBox,9);stroke(colorBox,C.cyan,.58);local p2=Instance.new("UIPadding");p2.PaddingLeft=UDim.new(0,10);p2.Parent=colorBox
local preview=Instance.new("Frame");preview.Position=UDim2.fromOffset(144,120);preview.Size=UDim2.new(1,-160,0,68);preview.BackgroundColor3=C.panel;preview.BorderSizePixel=0;preview.ZIndex=202;preview.Parent=panel;corner(preview,9);stroke(preview,C.muted,.6)
...
local save=btn(panel,"SAVE",UDim2.fromOffset(16,202),UDim2.fromOffset(96,38),C.pink);save.ZIndex=204
local equip=btn(panel,"EQUIP",UDim2.fromOffset(122,202),UDim2.fromOffset(96,38),Color3.fromRGB(45,86,72));equip.ZIndex=204
local remove=btn(panel,"REMOVE",UDim2.fromOffset(228,202),UDim2.fromOffset(96,38),Color3.fromRGB(83,38,47));remove.ZIndex=204
local status=text(panel,"TITLE not loaded yet.",UDim2.fromOffset(16,250),UDim2.new(1,-32,0,68),Enum.Font.Gotham,10,C.muted);status.TextWrapped=true;status.TextYAlignment=Enum.TextYAlignment.Top;status.ZIndex=202

local camera=workspace.CurrentCamera
local function layout()
 camera=workspace.CurrentCamera or camera;local vp=camera and camera.ViewportSize or Vector2.new(1280,720)
 panel.Size=UDim2.fromOffset(math.min(340,math.max(260,vp.X-24)),math.min(340,math.max(280,vp.Y-64)))
 panel.Position=UDim2.new(1,-12,.5,20)
end
...
local function injectMenuButton()
 ...
 b.Parent=list;b:SetAttribute("BBYAInjectedAuthority","IDENTITY_UI_V1_2");b.Activated:Connect(function()if drawer and drawer:IsA("GuiObject") then drawer.Visible=false end;kernelMenuVisible(false);layout();shade.Visible=true;loadSnapshot()end);menuButton=b;return true
end
...
-- Level Up notification logic preserved.
...
print("[BBYA] Identity UI v1.2 online: General Panel lock / TITLE actions preserved / level logic preserved")