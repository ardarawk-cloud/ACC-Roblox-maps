...
local function addRearServiceDoor(parent, prefix, body, xRatio, accent)
    ...
    local made = 0
    if visualPart(parent, prefix .. "RearServiceDoor", Vector3.new(5.8, 8.4, 0.42), body.CFrame * CFrame.new(x, baseY, rearZ), C.charcoal, Enum.Material.Metal) then made += 1 end
    if visualPart(parent, prefix .. "RearDoorHeader", Vector3.new(6.6, 0.55, 0.55), body.CFrame * CFrame.new(x, baseY + 4.45, rearZ - 0.06), accent or C.navy, Enum.Material.Metal) then made += 1 end
    if visualPart(parent, prefix .. "RearMeterBox", Vector3.new(2.4, 3.0, 0.55), body.CFrame * CFrame.new(x + 4.5, baseY + 0.5, rearZ), C.silver, Enum.Material.Metal) then made += 1 end
    return made
end
...
local function addRearLouver(parent, prefix, body, xRatio, width)
    ...
    local made = 0
    if visualPart(parent, prefix .. "RearLouverFrame", Vector3.new(w, 4.6, 0.38), body.CFrame * CFrame.new(x, y, rearZ), C.metal, Enum.Material.Metal) then made += 1 end
    for i = -2, 2 do
        if visualPart(parent, prefix .. "RearLouverBlade", Vector3.new(w - 0.7, 0.28, 0.24), body.CFrame * CFrame.new(x, y + i * 0.72, rearZ - 0.28), C.charcoal, Enum.Material.Metal) then made += 1 end
    end
    return made
end
...
if school then
    ...
    for index, buildingName in ipairs({"MainBuilding", "LeftWing", "RightWing"}) do
        ...
        if body and body:IsA("BasePart") then
            schoolDetailCount += addRearServiceDoor(schoolDetail, buildingName, body, index == 2 and -0.24 or 0.24, C.blue)
            schoolDetailCount += addRearLouver(schoolDetail, buildingName, body, index == 3 and 0.18 or -0.18, buildingName == "MainBuilding" and 9.5 or 6.8)
            ...
            for _, side in ipairs({-1, 1}) do
                for _, yRatio in ipairs({-0.18, 0.18}) do
                    local panel = visualPart(schoolDetail, buildingName .. "SideRelief", Vector3.new(0.42, math.max(4.5, body.Size.Y * 0.22), math.max(5.0, body.Size.Z * 0.34)), body.CFrame * CFrame.new(sideX * side, body.Size.Y * yRatio, 0), C.navy, Enum.Material.Metal)
...
local shopAccents = {ARCADE = C.gold, CAFE = C.teal, STYLE = C.purple, MUSIC = C.blue, HOBBY = C.red}
if downtown then
    ...
    for shopName, accent in pairs(shopAccents) do
        ...
        if body and body:IsA("BasePart") then
            ...
            if visualPart(downtownDetail, shopName .. "RearConduit", Vector3.new(0.36, math.max(5, body.Size.Y - 5), 0.36), body.CFrame * CFrame.new(conduitX, 0, rearZ), C.metal, Enum.Material.Metal) then downtownDetailCount += 1 end
        end
    end
end
...
for _, snapshot in pairs(protectedRoadSnapshot) do
    ...
    if p.CFrame ~= snapshot.CFrame or p.Size ~= snapshot.Size or p.Material ~= snapshot.Material or p.Color ~= snapshot.Color then roadsUnchanged = false break end
end
local poolUnchanged = true
for _, snapshot in pairs(protectedPoolSnapshot) do
    local p = snapshot.Ref
    if p.CFrame ~= snapshot.CFrame or p.Size ~= snapshot.Size or p.Material ~= snapshot.Material then poolUnchanged = false break end
end
local visualSafetyPass = true
for _, obj in ipairs(layer:GetDescendants()) do
    if obj:IsA("BasePart") and (obj.CanCollide or obj.CanTouch or obj.CanQuery) then visualSafetyPass = false break end
end
...
print(string.format("[AFTER SCHOOL CITY] V0.9.3 final environment polish ready; parts=%d school=%d downtown=%d studentRow=%d vegetation=%d recreation=%d", partCount, schoolDetailCount, downtownDetailCount, studentRowDetailCount, vegetationDetailCount, recreationDetailCount))
