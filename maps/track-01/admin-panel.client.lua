...
panel.Size=UDim2.fromScale(0.86,0.66)
...
panelSize.MaxSize=Vector2.new(440,470)
...
toast.Size=UDim2.new(0.84,0,0,30)
...
local function createPage(name,canvasHeight)
    ...
    page.CanvasSize=UDim2.fromOffset(0,canvasHeight or 300)
...
local statusPage=createPage("STATUS",310)
local paPage=createPage("PA",305)
local lightsPage=createPage("LIGHTS",295)
local playersPage=createPage("PLAYERS",320)

helperText(statusPage,"View current server state, venue mode, event mode, lighting and player count.")
local statusCard=Instance.new("Frame")
statusCard.Position=UDim2.fromOffset(2,34)
statusCard.Size=UDim2.new(1,-10,0,96)
...
sectionLabel(statusPage,"VENUE MODE",140)
actionButton(statusPage,"NIGHT SERVICE",UDim2.new(0,2,0,164),UDim2.new(0.33,-5,0,38),C.green,function()
    command:FireServer("VENUE_STATUS","NIGHT_SERVICE"); notify("Venue: Night Service")
end)
actionButton(statusPage,"BOARDING HOLD",UDim2.new(0.33,2,0,164),UDim2.new(0.34,-5,0,38),C.amber,function()
    command:FireServer("VENUE_STATUS","BOARDING_HOLD"); notify("Venue: Boarding Hold")
end)
actionButton(statusPage,"CLOSING",UDim2.new(0.67,2,0,164),UDim2.new(0.33,-8,0,38),C.red,function()
    command:FireServer("VENUE_STATUS","CLOSING"); notify("Closing PA queued")
end)

sectionLabel(statusPage,"EVENT MODE",212)
actionButton(statusPage,"END OF LINE",UDim2.new(0,2,0,236),UDim2.new(0.33,-5,0,38),C.red,function()
    command:FireServer("EVENT_MODE","END_OF_LINE"); notify("Event: End of Line")
end)
actionButton(statusPage,"LAST TRAIN",UDim2.new(0.33,2,0,236),UDim2.new(0.34,-5,0,38),C.amber,function()
    command:FireServer("EVENT_MODE","LAST_TRAIN"); notify("Event: Last Train")
end)
actionButton(statusPage,"CLEAR",UDim2.new(0.67,2,0,236),UDim2.new(0.33,-8,0,38),C.muted,function()
...
actionButton(paPage,"BOARDING CALL",UDim2.new(0,2,0,64),UDim2.new(0.5,-7,0,48),C.amber,function()
    command:FireServer("PA_PRESET","BOARDING"); notify("Boarding PA queued")
end)
actionButton(paPage,"NIGHT SERVICE",UDim2.new(0.5,3,0,64),UDim2.new(0.5,-9,0,48),C.green,function()
    command:FireServer("PA_PRESET","NIGHT_SERVICE"); notify("Night Service PA queued")
end)
actionButton(paPage,"END OF LINE",UDim2.new(0,2,0,120),UDim2.new(0.5,-7,0,48),C.red,function()
    command:FireServer("PA_PRESET","END_OF_LINE"); notify("End of Line PA queued")
end)
actionButton(paPage,"CLOSING",UDim2.new(0.5,3,0,120),UDim2.new(0.5,-9,0,48),C.red,function()
...
paNote.Text="PA CONTROL\nManual presets only. No uploaded audio asset is added. If another announcement is speaking, the manual PA waits briefly before playing."
...
paPad.PaddingTop=UDim.new(0,8)
paPad.PaddingLeft=UDim.new(0,9)
paPad.PaddingRight=UDim.new(0,9)
paPad.Parent=paNote

helperText(lightsPage,"Adjust venue readability and mood. These controls do not change the music source.")
sectionLabel(lightsPage,"LIGHTING PRESETS",40)
actionButton(lightsPage,"STANDARD",UDim2.new(0,2,0,64),UDim2.new(0.33,-5,0,46),C.green,function()
    command:FireServer("LIGHTING_PRESET","STANDARD"); notify("Lighting: Standard")
end)
actionButton(lightsPage,"READABLE",UDim2.new(0.33,2,0,64),UDim2.new(0.34,-5,0,46),C.amber,function()
    command:FireServer("LIGHTING_PRESET","READABLE"); notify("Lighting: Readable")
end)
actionButton(lightsPage,"LOW NIGHT",UDim2.new(0.67,2,0,64),UDim2.new(0.33,-8,0,46),C.muted,function()
    command:FireServer("LIGHTING_PRESET","LOW_NIGHT"); notify("Lighting: Low Night")
end)

sectionLabel(lightsPage,"DJ / CLUB PULSE",122)
actionButton(lightsPage,"PULSE ON",UDim2.new(0,2,0,146),UDim2.new(0.5,-7,0,44),C.amber,function()
    command:FireServer("PULSE",true); notify("Club pulse enabled")
end)
actionButton(lightsPage,"PULSE OFF",UDim2.new(0.5,3,0,146),UDim2.new(0.5,-9,0,44),C.muted,function()
...
local function rebuildPlayers()
    ...
    for _,target in ipairs(targets) do
        ...
        actionButton(row,"LOBBY",UDim2.new(0.47,0,0,6),UDim2.new(0.25,-5,0,34),C.amber,function()
            command:FireServer("RETURN_LOBBY",target.UserId); notify("Returned "..target.DisplayName.." to lobby")
        end)
        actionButton(row,"RESPAWN",UDim2.new(0.72,2,0,6),UDim2.new(0.28,-7,0,34),C.red,function()
...
local function applyState(data)
    ...
    statusBody.Text=string.format("VENUE  %s\nEVENT  %s   •   LIGHTS  %s\nPULSE  %s   •   SERVER  %s\nPLAYERS  %s",venue,event,lights,pulse,feature,tostring(data.playerCount or #Players:GetPlayers()))
end
...
print("[TRACK 01] compact mobile admin panel client ready v4.0.1")
