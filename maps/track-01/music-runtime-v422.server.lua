-- TRACK 01 v4.2.2 — AUDIO STABILITY / LEVEL HOTFIX
-- Scope: preserve the 62-track permissioned pool, Car 01-04 only, one centered emitter per car.
-- Changes: stronger default level, equal-power crossfade, preloaded deck start, follower recovery.
...
local VERSION="4.2.2"
local CROSSFADE=7
local MIX_LEAD=CROSSFADE+2.2
local MASTER_DEFAULT=1.00
...
assert(#PLAYLIST==62,"TRACK01 62-track permissioned catalog lock failed")
...
for _,spec in ipairs(CAR_SPECS) do
    ...
    p.Name="EmitterCenter";p.Size=Vector3.new(.25,.25,.25);p.CFrame=CFrame.new(25.5,9.2,spec.z)
    p.Anchored=true;p.CanCollide=false;p.CanTouch=false;p.CanQuery=false;p.Transparency=1;p.Parent=f
    for _,deckName in ipairs({"A","B"}) do
        local s=Instance.new("Sound")
        s.Name="Deck"..deckName;s.Volume=0;s.Looped=false;s.RollOffMode=Enum.RollOffMode.InverseTapered
        s.RollOffMinDistance=ROLLOFF_MIN;s.RollOffMaxDistance=ROLLOFF_MAX;s.EmitterSize=EMITTER_SIZE;s.Parent=p
...
local function authorized(p)
    ...
    if type(raw)=="string" then
        for token in string.gmatch(raw,"[^,%s]+") do if tonumber(token)==p.UserId then return true end end
    end
...
local function setVolume(deckName,a)
    a=math.clamp(a,0,1)
    for _,r in ipairs(decks[deckName]) do
        r.sound.Volume=math.clamp(masterVolume*r.gain*a,0,1)
    end
end
local function stopDeck(deckName)
    for _,r in ipairs(decks[deckName]) do
        pcall(function()r.sound:Stop()end);r.sound.TimePosition=0;r.sound.Volume=0
    end
end
local function rep(deckName)
    return decks[deckName][1] and decks[deckName][1].sound or nil
end
local function alignDeck(deckName,threshold)
    ...
    for i=2,#decks[deckName] do
        ...
        if not s.IsPlaying then
            pcall(function()s.TimePosition=t;s:Play()end)
        elseif math.abs((s.TimePosition or 0)-t)>(threshold or .12) then
            pcall(function()s.TimePosition=t end)
        end
    end
end
...
local function prepare(deckName,i)
    local t=PLAYLIST[i]
    probe:Stop();probe.SoundId="rbxassetid://"..t.id;probe.TimePosition=0
    local probeOk=pcall(function()ContentProvider:PreloadAsync({probe})end)
    if not probeOk then
        failedUntil[i]=os.clock()+60;Workspace:SetAttribute("TRACK01_MUSIC_LAST_FAILED_ID",t.id);return false
    end
    local sounds={}
    for _,r in ipairs(decks[deckName]) do
        local s=r.sound
        pcall(function()s:Stop()end)
        s.SoundId="rbxassetid://"..t.id;s.TimePosition=0;s.Volume=0
        s:SetAttribute("TrackId",t.id);s:SetAttribute("TrackSource",t.source)
        table.insert(sounds,s)
    end
    local ok=pcall(function()ContentProvider:PreloadAsync(sounds)end)
    if not ok then
        failedUntil[i]=os.clock()+60;Workspace:SetAttribute("TRACK01_MUSIC_LAST_FAILED_ID",t.id);return false
    end
    ...
    while os.clock()<deadline do
        local ready=true
        for _,s in ipairs(sounds) do if not s.IsLoaded or (s.TimeLength or 0)<=1 then ready=false;break end end
        if ready then return true end
        task.wait(.08)
    end
    local lead=sounds[1]
    if lead and lead.IsLoaded and (lead.TimeLength or 0)>1 then return true end
    failedUntil[i]=os.clock()+60;Workspace:SetAttribute("TRACK01_MUSIC_LAST_FAILED_ID",t.id);return false
end
...
local function playDeck(deckName)
    local count=0
    for _,r in ipairs(decks[deckName]) do
        if pcall(function()r.sound.TimePosition=0;r.sound:Play()end) then count+=1 end
    end
    if count==0 then return false end
    local deadline=os.clock()+1.2
    repeat
        local lead=rep(deckName)
        if lead and lead.IsPlaying then
            task.wait(.08);alignDeck(deckName,.20);return true
        end
        task.wait(.04)
...
local function publish()
    local r=rep(active)
    Workspace:SetAttribute("TRACK01_MUSIC_AUTO",true)
    Workspace:SetAttribute("TRACK01_MUSIC_AUTOMIX",true)
    Workspace:SetAttribute("TRACK01_MUSIC_SPATIAL_CARS_ONLY",true)
    Workspace:SetAttribute("TRACK01_MUSIC_SKATEPARK_EXCLUDED",true)
    Workspace:SetAttribute("TRACK01_MUSIC_EMITTERS_PER_CAR",1)
    Workspace:SetAttribute("TRACK01_MUSIC_PHASE_SAFE",true)
    Workspace:SetAttribute("TRACK01_MUSIC_ROLLOFF_MIN",ROLLOFF_MIN)
    Workspace:SetAttribute("TRACK01_MUSIC_ROLLOFF_MAX",ROLLOFF_MAX)
    Workspace:SetAttribute("TRACK01_MUSIC_TRACK_COUNT",#PLAYLIST)
    Workspace:SetAttribute("TRACK01_MUSIC_VERSION",VERSION)
    Workspace:SetAttribute("TRACK01_MUSIC_VOLUME",masterVolume)
    Workspace:SetAttribute("TRACK01_MUSIC_STABILITY_MODE","PRELOAD_EQUAL_POWER_RECOVERY")
    Workspace:SetAttribute("TRACK01_MUSIC_STATUS",paused and "PAUSED" or (currentTrack and "PLAYING" or "STARTING"))
    Workspace:SetAttribute("TRACK01_MUSIC_CURRENT_SOUND_ID",currentTrack and currentTrack.id or "")
    Workspace:SetAttribute("TRACK01_MUSIC_CURRENT_TRACK",currentTrack and ("BBYA "..currentTrack.source) or "AUTO RANDOM 24/7")
    Workspace:SetAttribute("TRACK01_MUSIC_CURRENT_SOURCE",currentTrack and currentTrack.source or "")
    Workspace:SetAttribute("TRACK01_MUSIC_PLAYING",r and r.IsPlaying and not paused or false)
    if not stateEvent or not stateEvent.Parent then
        local folder=ReplicatedStorage:FindFirstChild("TRACK01_Admin");stateEvent=folder and folder:FindFirstChild("State") or nil
    end
...
-- Equal-power crossfade prevents the audible midpoint dip of the previous linear fade.
local function fade(fromDeck,toDeck)
    local steps=70
    for i=0,steps do
        while paused do task.wait(.15) end
        local a=i/steps
        local fromAmp=math.cos(a*math.pi*0.5)
        local toAmp=math.sin(a*math.pi*0.5)
        setVolume(fromDeck,fromAmp);setVolume(toDeck,toAmp)
        task.wait(CROSSFADE/steps)
    end
end
...
local function first()
    for _=1,#PLAYLIST do
        local i=candidate()
        if prepare(active,i) and playDeck(active) then
            currentIndex=i;currentTrack=PLAYLIST[i]
            for n=1,15 do setVolume(active,math.sin((n/15)*math.pi*0.5));task.wait(.07) end
            publish();return true
        end
    end
    return false
...
local function nextMix()
    ...
    for _=1,#PLAYLIST do
        local i=candidate()
        if prepare(inactive,i) and playDeck(inactive) then chosen=i;break end
    end
    if not chosen then transitioning=false;return false end
    local old,new=active,inactive
    local oldLead=rep(old)
    if oldLead and oldLead.IsPlaying then
        fade(old,new)
    else
        setVolume(new,1)
    end
    stopDeck(old)
    active,inactive=new,old;currentIndex=chosen;currentTrack=PLAYLIST[chosen]
    transitioning=false;publish();return true
end

local function restart()
    if currentIndex<1 then return end
    for _,r in ipairs(decks[active]) do
        r.sound.TimePosition=0
        if not paused then pcall(function()r.sound:Play()end) end
    end
    setVolume(active,1)
    task.defer(function()task.wait(.10);alignDeck(active,.20)end)
    publish()
end
local function pauseAll()
    for _,d in ipairs({"A","B"}) do for _,r in ipairs(decks[d]) do pcall(function()r.sound:Pause()end) end end
end
local function resumeAll()
    for _,d in ipairs({"A","B"}) do for _,r in ipairs(decks[d]) do pcall(function()r.sound:Resume()end) end end
    task.defer(function()task.wait(.10);alignDeck(active,.20);if transitioning then alignDeck(inactive,.20) end end)
end

task.spawn(function()
    while task.wait(.4) do
        local folder=ReplicatedStorage:FindFirstChild("TRACK01_Admin")
        local command=folder and folder:FindFirstChild("Command")
        if command and command:IsA("RemoteEvent") then
            command.OnServerEvent:Connect(function(p,a,v)
                ...
                if a=="MUSIC_PLAY" then
                    paused=false;resumeAll();if not (rep(active) and rep(active).IsPlaying) then restart() end
                elseif a=="MUSIC_PAUSE" then
                    paused=true;pauseAll()
                elseif a=="MUSIC_RESTART" then
                    restart()
                elseif a=="MUSIC_VOLUME" and type(v)=="number" then
                    masterVolume=math.clamp(masterVolume+v,.10,1);setVolume(active,1)
                end
...
task.spawn(function()
    ...
    while task.wait(.25) do
        if not paused and not transitioning then
            local r=rep(active)
            if not r or not r.IsPlaying then
                nextMix()
            else
                local len=r.TimeLength or 0
                if len>2 and r.TimePosition>=math.max(1,len-MIX_LEAD) then nextMix() end
            end
        end
    end
end)

-- Low-frequency recovery only; avoids constant micro-seeking while restoring a dropped car emitter.
task.spawn(function()
    while task.wait(4) do
        if not paused then
            alignDeck(active,.18)
            if transitioning then alignDeck(inactive,.18) end
            setVolume(active,1)
        end
    end
end)
task.spawn(function()while task.wait(2) do publish() end end)
print("[TRACK 01] v4.2.2 AutoDJ ready: louder default, equal-power automix, preloaded synchronized car emitters, 62-track permissioned pool")
